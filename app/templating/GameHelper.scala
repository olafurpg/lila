...
trait GameHelper { self: I18nHelper with UserHelper with AiHelper with StringHelper =>
  ...
  def gameSummary(
      whiteUserId: String, blackUserId: String, finished: Boolean, result: Option[Boolean]) = {
    val res =
      if (finished)
        result match {
          case Some(true) => "1-0"
  ...
  def gameLink(game: Game, color: Color, ownerLink: Boolean = false, tv: Boolean = false)(
