...
trait UserHelper { self: I18nHelper with StringHelper with NumberHelper =>
  ...
  def showProgress(progress: Int, withTitle: Boolean = true) = Html {
    ...
    val title =
      if (withTitle) """data-hint="Rating progression over the last twelve games"""" else ""
  ...
  def showPerfRating(
      rating: Int, name: String, nb: Int, provisional: Boolean, icon: Char, klass: String)(
  ...
  def showPerfRating(perfType: PerfType, perf: Perf, klass: String)(implicit ctx: Context): Html =
    showPerfRating(
      perf.intRating, perfType.name, perf.nb, perf.provisional, perfType.iconChar, klass)
  ...
  def userGameFilterTitleNoTag(info: UserInfo, filter: GameFilter)(implicit ctx: UserContext) =
    Html((filter match {
