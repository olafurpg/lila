...
object GameFilterMenu {
  ...
  val all =
    NonEmptyList.nel(All, List(Me, Rated, Win, Loss, Draw, Playing, Bookmark, Imported, Search))

  def apply(info: UserInfo, me: Option[User], currentNameOption: Option[String]): GameFilterMenu = {
  ...
  private def cachedNbOf(user: User, info: Option[UserInfo], filter: GameFilter): Option[Int] =
    filter match {
  ...
  def paginatorOf(
    ...
    filter match {
      ...
      case Imported =>
        pag.apply(selector = Query.imported(user.id), sort = $sort.desc("pgni.ca"), nb = nb)(page)
      ...
      case Playing =>
        pag(selector = Query.nowPlaying(user.id), sort = $empty, nb = nb)(page).addEffect { p =>
  ...
  def searchForm(userGameSearch: lila.gameSearch.UserGameSearch, filter: GameFilter)(
