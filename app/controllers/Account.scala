...
object Account extends LilaController {
  ...
  def info = Open { implicit ctx =>
    negotiate(
      ...
      api = _ =>
          ...
          case Some(me) =>
            relationEnv.api.countFollowers(me.id) zip relationEnv.api.countFollowing(me.id) zip Env.pref.api
  ...
  private def emailForm(user: UserModel) = UserRepo email user.id map { email =>
    Env.security.forms.changeEmail(user).fill(lila.security.DataForm.ChangeEmail(~email, ""))
  }
  ...
  def security = Auth { implicit ctx => me =>
    Env.security.api.dedup(me.id, ctx.req) >> Env.security.api.locatedOpenSessions(me.id, 50) map {
      sessions =>
