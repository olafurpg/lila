...
object Puzzle extends LilaController {
  ...
  private def puzzleJson(puzzle: PuzzleModel)(implicit ctx: Context) =
    (env userInfos ctx.me) zip (ctx.me ?? { env.api.attempt.hasPlayed(_, puzzle) map (!_) }) map {
      case (infos, asPlay) =>
        JsData(
          puzzle, infos, asPlay.fold("play", "try"), animationDuration = env.AnimationDuration)
    }
  ...
  def frame = Open { implicit ctx =>
    OptionOk(env.daily()) { daily =>
      html.puzzle.embed(daily, get("bg") | "light", lila.pref.Theme(~get("theme")).cssClass)
    }
  }
}
