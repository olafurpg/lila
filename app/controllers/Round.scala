...
object Round extends LilaController with TheftPrevention {
  ...
  private def renderPlayer(pov: Pov)(implicit ctx: Context): Fu[Result] =
    negotiate(
      html = pov.game.started.fold(
          PreventTheft(pov) {
          myTour(pov.game.tournamentId, true) zip (pov.game.simulId ?? Env.simul.repo.find) zip Env.game
            .crosstableApi(pov.game) zip (pov.game.isSwitchable ?? otherPovs(pov.game)) flatMap {
        ...
        negotiate(
          html = {
            if (getBool("sudo") && isGranted(_.SuperAdmin))
              ...
              myTour(pov.game.tournamentId, false) zip (pov.game.simulId ?? Env.simul.repo.find) zip Env.game
                .crosstableApi(pov.game) zip Env.api.roundApi.watcher(
                pov, lila.api.Mobile.Api.currentVersion, tv = none, withOpening = false) map {
              ...
              GameRepo.initialFen(pov.game.id) zip Env.game.crosstableApi(pov.game) map {
          ...
          api = apiVersion =>
              Env.api.roundApi.watcher(pov, apiVersion, tv = none, withOpening = false) map {
              Ok(_)
          }
        ) map NoCache
    }
  ...
  private def sides(pov: Pov, isPlayer: Boolean)(implicit ctx: Context) =
    myTour(pov.game.tournamentId, isPlayer) zip (pov.game.simulId ?? Env.simul.repo.find) zip GameRepo
      .initialFen(pov.game) zip Env.game.crosstableApi(pov.game) map {
  ...
  def continue(id: String, mode: String) = Open { implicit ctx =>
    OptionResult(GameRepo game id) { game =>
      Redirect("%s?fen=%s#%s".format(routes.Lobby.home(),
