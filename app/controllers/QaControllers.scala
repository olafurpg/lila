...
trait QaController extends LilaController {
  ...
  protected def renderQuestion(q: Question, answerForm: Option[Form[_]] = None)(
    ...
    (api.answer
      .popular(q.id))
      .zip(fetchPopular)
      .zip(api.relation.questions(q, 10))
          ...
          fuccess {
            Ok(
              views.html.qa.questionShow(
                ...
                related,
                answerForm =
                  if (QaAuth.canAnswer(q)) answerForm.orElse(Some(forms.answer)) else None,
                captcha = captcha))
          }
        case _ => notFound
      }
  ...
  protected def WithQuestion(id: QuestionId, slug: String)(block: Question => Fu[Result])(
      implicit ctx: Context): Fu[Result] =
    WithQuestion(id) { q =>
  ...
  protected def WithOwnQuestion(id: QuestionId, slug: String)(block: Question => Fu[Result])(
      implicit ctx: Context): Fu[Result] =
  ...
  protected def WithOwnAnswer(questionId: QuestionId, answerId: AnswerId)(
      block: Question => Answer => Fu[Result])(implicit ctx: Context): Fu[Result] =
