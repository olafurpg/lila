...
object Analyse extends LilaController {
      ...
      GameRepo initialFen pov.game.id flatMap { initialFen =>
        RedirectAtFen(pov, initialFen) {
          (env.analyser get pov.game.id) zip Env.fishnet.api
            .prioritaryAnalysisInProgress(pov.game.id) zip
          (pov.game.simulId ?? Env.simul.repo.find) zip Env.game.crosstableApi(pov.game) flatMap {
              ...
              Env.api.roundApi.watcher(pov,
                ...
                Ok(
                  html.analyse.replay(
                    ...
                    analysis,
                    analysis map { a =>
                  AdvantageChart(a.infoAdvices, pov.game.pgnMoves, pov.game.startedAtTurn)
                },
                    analysisInProgress,
                    simul,
  ...
  private def RedirectAtFen(pov: Pov, initialFen: Option[String])(or: => Fu[Result])(
  ...
  private def replayBot(pov: Pov)(implicit ctx: Context) =
    GameRepo initialFen pov.game.id flatMap { initialFen =>
      (env.analyser get pov.game.id) zip (pov.game.simulId ?? Env.simul.repo.find) zip Env.game
