...
object Tournament extends LilaController {
      ...
      html = Reasonable(page, 20) {
        ...
        if (HTTPRequest.isXhr(ctx.req)) finishedPaginator.map { pag =>
          ...
          env.api.fetchVisibleTournaments
            ...
            .zip(UserRepo.allSortToints(10))
            .map {
              case (((visible, scheduled), finished), leaderboard) =>
                Ok(html.tournament
                  .home(scheduled, finished, leaderboard, env.scheduleJsonView(visible)))
            }
            .map(NoCache)
      },
      api = _ =>
          ...
          .flatMap {
            ...
            case Some(tour) => {
              get("playerInfo")
                ...
                .zip(getBool("socketVersion").??(env.version(tour.id).map(some)))
                .flatMap {
  ...
  private def withUserGameNb(id: String, user: String, nb: Int)(withPov: Pov => Result)(
