...
object Tv extends LilaController {
        ...
        OptionFuResult(GameRepo.pov(gameId, color)) { pov =>
          Env.tv.tv.getChampions zip Env.game.crosstableApi(pov.game) map {
            case (champions, crosstable) =>
              Ok(html.tv.sides(channel, champions, pov, crosstable, streams = Nil))
          }
        }
    }
  }
      ...
      negotiate(
        html = {
          Env.api.roundApi.watcher(
            ...
            tv = onTv.some,
            withOpening = false) zip Env.game.crosstableApi(game) zip Env.tv.tv.getChampions map {
        ...
        api = apiVersion =>
            Env.api.roundApi.watcher(pov, apiVersion, tv = onTv.some, withOpening = false) map {
            Ok(_)
        }
      )
    }
  ...
  def feed = Action.async {
    ...
    Env.round.tvBroadcast ? TvBroadcast.GetEnumerator mapTo manifest[TvBroadcast.EnumeratorType] map {
      enum =>
