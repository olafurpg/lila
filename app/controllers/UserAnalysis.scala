...
object UserAnalysis extends LilaController with TheftPrevention {
  ...
  def load(urlFen: String, variant: Variant) = Open { implicit ctx =>
    ...
    Env.api.roundApi.userAnalysisJson(pov, ctx.pref, decodedFen, orientation, owner = false).map {
      data =>
  ...
  private def makePov(from: SituationPlus) =
    lila.game.Pov(
      lila.game.Game
        .make(
          game = chess
            .Game(board = from.situation.board, player = from.situation.color, turns = from.turns),
    ...
    OptionFuResult(GameRepo.game(id)) { game =>
      GameRepo
        ...
        .flatMap { initialFen =>
          ...
          Env.api.roundApi
            .userAnalysisJson(pov, ctx.pref, initialFen, pov.color, owner = isMyPov(pov))
            .map { data =>
              Ok(html.board.userAnalysis(data, pov))
            }
        }
          ...
          Env.importer.importer
            ...
            .fold(
              ...
              game => {
                ...
                Env.api.roundApi
                  .userAnalysisJson(pov, ctx.pref, initialFen = none, pov.color, owner = false)
                  .map { data =>
                    Ok(data)
                  }
              }
  ...
  def forecastsOnMyTurn(fullId: String, uci: String) = AuthBody(BodyParsers.parse.json) {
    implicit ctx => me =>
      import lila.round.Forecast
      OptionFuResult(GameRepo.pov(fullId)) { pov =>
        if (isTheft(pov)) fuccess(theftResponse)
