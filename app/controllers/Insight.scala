...
object Insight extends LilaController {
  ...
  def index(username: String) =
    path(username, metric = Metric.MeanCpl.key, dimension = Dimension.Perf.key, filters = "")
        ...
        .fold(
          ...
          qJson =>
            qJson.question.fold(BadRequest.fuccess) { q =>
              env.api.ask(q, user) map lila.insight.Chart.fromAnswer(Env.user.lightUser) map env.jsonView.chart.apply map {
                Ok(_)
              }
          }
        )
