...
object QaAnswer extends QaController {
    ...
    forms.vote.bindFromRequest.fold(
      ...
      v =>
        api.answer.vote(answerId, me, v == 1).map {
          case Some(vote) =>
            Ok(html.qa.vote(routes.QaAnswer.vote(questionId, answerId).url, vote, true))
          case None => NotFound
      }
    )
  }
  ...
  def remove(questionId: QuestionId, answerId: AnswerId) = Secure(_.ModerateQa) {
    implicit ctx => me =>
      OptionFuRedirect(api.answer.findById(answerId)) { a =>
        (api.answer.remove(a.id)) >>
          Env.mod.logApi
            .deleteQaAnswer(me.id, a.userId, a.body)
    ...
    WithOwnAnswer(questionId, answerId) { q => a =>
      ...
      forms.moveAnswer.bindFromRequest.fold(
        err => renderQuestion(q), {
          case "question" =>
            api.answer
              .moveToQuestionComment(a, q)
              .inject(Redirect(routes.QaQuestion.show(q.id, q.slug)))
          case str =>
            parseIntOption(str).fold(renderQuestion(q)) { answerId =>
              api.answer
                .moveToAnswerComment(a, answerId)
                .inject(Redirect(routes.QaQuestion.show(q.id, q.slug)))
            }
        }
      )
