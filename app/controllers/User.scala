...
object User extends LilaController {
  ...
  private def filter(username: String,
    ...
    Reasonable(page) {
      OptionFuResult(UserRepo named username) { u =>
        if (u.enabled || isGranted(_.UserSpy))
          ...
          negotiate(html = fuccess(NotFound(html.user.disabled(u))),
  ...
  def mod(username: String) = Secure(_.UserSpy) { implicit ctx => me =>
    OptionFuOk(UserRepo named username) { user =>
      (!isGranted(_.SetEmail, user) ?? UserRepo.email(user.id)) zip (Env.security userSpy user.id) zip
      ...
      if ((u.disabled || (u.lame && !ctx.is(u))) && !isGranted(_.UserSpy)) notFound
        ...
        lila.rating.PerfType(perfKey).fold(notFound) { perfType =>
          for {
            ...
            response <- negotiate(html = Ok(html.user.perfStat(u, ranks, perfType, data)).fuccess,
