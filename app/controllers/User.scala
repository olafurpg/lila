  ...
  def mod(username: String) = Secure(_.UserSpy) { implicit ctx => me =>
    OptionFuOk(UserRepo.named(username)) { user =>
      (!isGranted(_.SetEmail, user) ?? UserRepo.email(user.id))
        ...
        .zip(Env.mod.assessApi.getPlayerAggregateAssessmentWithGames(user.id))
        .zip(Env.mod.logApi.userHistory(user.id))
        .flatMap {
    ...
    OptionFuResult(UserRepo.named(username)) { user =>
      ...
      env.forms.note.bindFromRequest.fold(
        ...
        text =>
          env.noteApi
            .write(user, text, me)
            .inject(Redirect(routes.User.show(username).url + "?note"))
      )
    }
  }
