...
object Team extends LilaController {
  ...
  private lazy val teamInfo = mashup.TeamInfo(api = api,
  ...
  def close(id: String) = Secure(_.CloseTeam) { implicit ctx => me =>
    OptionFuResult(api team id) { team =>
      (api delete team) >> Env.mod.logApi.deleteTeam(me.id, team.name, team.description) inject Redirect(
        routes.Team all 1)
    }
  }
  ...
  private def OnePerWeek[A <: Result](me: UserModel)(a: => Fu[A])(
    ...
    api.hasCreatedRecently(me) flatMap { did =>
      (did && !Granter.superAdmin(me)) fold (Forbidden(views.html.team.createLimit()).fuccess, a)
    }
