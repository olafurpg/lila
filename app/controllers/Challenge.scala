...
object Challenge extends LilaController {
  ...
  def all = Auth { implicit ctx => me =>
    env.api allFor me.id map { all =>
      Ok {
        env.jsonView(all) + ("i18n" -> translations)
      } as JSON
    }
  }
  ...
  private def translations(implicit ctx: Context) =
    Env.i18n.jsDump.keysToObject(List(
  ...
  def accept(id: String) = Open { implicit ctx =>
    OptionFuResult(env.api byId id) { c =>
      isForMe(c) ?? env.api.accept(c, ctx.me).flatMap {
        ...
        case None =>
          negotiate(html = Redirect(routes.Round.watcher(c.id, "white")).fuccess,
  ...
  def restriction(user: lila.user.User)(implicit ctx: Context): Fu[Option[String]] = ctx.me match {
    ...
    case Some(me) =>
      Env.relation.api.fetchBlocks(user.id, me.id) flatMap {
        ...
        case false =>
          Env.pref.api getPref user zip Env.relation.api.fetchFollows(user.id, me.id) map {
            case (pref, follow) =>
              lila.pref.Pref.Challenge
                .block(me, user, pref.challenge, follow, fromCheat = me.engine && !user.engine)
          }
      }
  }
