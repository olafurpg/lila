...
final class JsonView(chatApi: lila.chat.ChatApi,
      ...
      case (((socket, opponentUser), takebackable), chat) =>
        ...
        Json
          .obj(
            ...
            "chat" -> chat.map { c =>
              JsArray(c.lines map {
              case lila.chat.UserLine(username, text, _) => Json.obj("u" -> username, "t" -> text)
              case lila.chat.PlayerLine(color, text) => Json.obj("c" -> color.name, "t" -> text)
            })
            },
  ...
  def watcherJson(pov: Pov,
    ...
    getSocketStatus(pov.game.id) zip getWatcherChat(pov.game, user) zip UserRepo.pair(
      pov.player.userId, pov.opponent.userId) map {
  ...
  def userAnalysisJson(pov: Pov, pref: Pref, orientation: chess.Color, owner: Boolean) =
    (pov.game.pgnMoves.nonEmpty ?? GameRepo.initialFen(pov.game)) map { initialFen =>
      ...
      Json.obj("game" -> Json.obj("id" -> gameId,
...
object JsonView {
  ...
  implicit val variantWriter: OWrites[chess.variant.Variant] = OWrites { v =>
    Json.obj("key" -> v.key, "name" -> v.name, "short" -> v.shortName, "title" -> v.title)
  }
