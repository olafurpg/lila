...
private[round] final class Finisher(messenger: Messenger,
  ...
  private def apply(
    ...
    if (game.nonAi && game.isCorrespondence) Color.all foreach notifyTimeline(prog.game)
    lila.mon.game.finish(status.name)()
    casualOnly.fold(
      GameRepo unrate prog.game.id inject prog.game.copy(mode = chess.Mode.Casual),
      fuccess(prog.game)
    ) flatMap { g =>
      proxy.save(prog) >> GameRepo.finish(id = g.id,
                                          winnerColor = winner,
  ...
  private def updateCountAndPerfs(finish: FinishGame): Funit =
    (!finish.isVsSelf && !finish.game.aborted) ?? {
      ...
      } zip (finish.white ?? incNbGames(finish.game)) zip (finish.black ?? incNbGames(finish.game)) void
    }
