...
def receive: Receive =
  ...
  system.actorOf(Props(classOf[Titivate], roundMap, hub.actor.bookmark), name = "titivate")

  lazy val takebacker = new Takebacker(messenger = messenger, uciMemo = uciMemo, prefApi = prefApi)
  ...
  def resign(pov: lila.game.Pov) {
    if (pov.game.abortable) roundMap ! Tell(pov.game.id, actorApi.round.Abort(pov.playerId))
    else if (pov.game.playable) roundMap ! Tell(pov.game.id, actorApi.round.Resign(pov.playerId))
  }
}
