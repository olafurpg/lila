...
final class PerfsUpdater(historyApi: HistoryApi, rankingApi: RankingApi) {
    ...
    PerfPicker.main(game) ?? { mainPerf =>
      (game.rated && game.finished && game.accountable && !white.lame && !black.lame) ?? {
        ...
        resetGameRatings
          ...
          .zip(UserRepo.setPerfs(black, perfsB, black.perfs))
          .zip(historyApi.add(white, game, perfsW))
          .zip(historyApi.add(black, game, perfsB))
          .zip(rankingApi.save(white.id, game.perfType, perfsW))
  ...
  private def mkPerfs(ratings: Ratings, perfs: Perfs, game: Game): Perfs = {
    ...
    val perfs1 = perfs.copy(
      ...
      racingKings =
        addRatingIf(game.ratingVariant.racingKings, perfs.racingKings, ratings.racingKings),
