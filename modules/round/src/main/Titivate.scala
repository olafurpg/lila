...
private[round] final class Titivate(roundMap: ActorRef, bookmark: ActorSelection) extends Actor {
          ...
          case (count, game) => {
              ...
              if (game.finished || game.isPgnImport) GameRepo unsetCheckAt game
                ...
                game.clock match {
                  ...
                  case None =>
                    val days =
                      game.daysPerTurn | game.hasAi.fold(Game.aiAbandonedDays, Game.abandonedDays)
