    ...
    chess.Replay.gameWhileValid(pgnMoves, initialFen, variant) match {
      case (games, error) =>
        ...
        JsArray(a
          .fold[Seq[Step]](steps) {
            case (pgn, analysis) =>
              applyAnalysisAdvices(id, applyAnalysisEvals(steps, analysis), pgn, analysis, variant)
          }
          .map(_.toJson))
    }
  }
    ...
    steps.zipWithIndex.map {
      case (step, index) =>
        analysis.infos.lift(index - 1).fold(step) { info =>
          step.copy(
            eval = Step
              .Eval(cp = info.score.map(_.ceiled.centipawns), mate = info.mate, best = info.best)
              .some)
        }
    }
