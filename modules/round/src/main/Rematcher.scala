...
private[round] final class Rematcher(messenger: Messenger,
  ...
  private def rematchJoin(pov: Pov): Fu[Events] =
    for {
      ...
      _ ← (GameRepo insertDenormalized nextGame) >> GameRepo.saveNext(pov.game, nextGame.id) >>- messenger
  ...
  private def rematchCreate(pov: Pov)(implicit proxy: GameProxy): Fu[Events] =
    proxy.save {
  ...
  private def returnGame(pov: Pov): Fu[Game] =
    for {
      ...
      Game.make(game = ChessGame(board = Board(pieces, variant = pov.game.variant).withCastles {
