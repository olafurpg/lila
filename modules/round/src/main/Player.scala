...
private[round] final class Player(fishnetPlayer: lila.fishnet.Player,
  ...
  def human(play: HumanPlay, round: ActorRef)(pov: Pov)(implicit proxy: GameProxy): Fu[Events] =
    play match {
      case p @ HumanPlay(playerId, uci, blur, lag, promiseOption) =>
        pov match {
          case Pov(game, color) if game playableBy color =>
            p.trace
              .segmentSync("applyUci", "logic")(applyUci(game, uci, blur, lag))
              ...
              .flatMap {
                case (progress, moveOrDrop) =>
                  ...
                  (pov.game.hasAi ! uciMemo.add(pov.game, moveOrDrop)) >>- notifyMove(
                    ...
                    .fold(moveFinish(progress.game, color) map { progress.events ::: _ }, {
    ...
    if (game.playable && game.player.isAi) {
      if (currentFen == FEN(Forsyth >> game.toChess))
        applyUci(game, uci, blur = false, lag = 0.millis)
          ...
          .flatMap {
            case (progress, moveOrDrop) =>
              proxy.save(progress) >>- uciMemo.add(progress.game, moveOrDrop) >>- notifyMove(
