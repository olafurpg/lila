...
final class Env(config: Config, system: ActorSystem, db: lila.db.Env) {
  ...
  lazy val paginator = new PaginatorBuilder(coll = bookmarkColl, maxPerPage = PaginatorMaxPerPage)

  lazy val api = new BookmarkApi(coll = bookmarkColl, cached = cached, paginator = paginator)
