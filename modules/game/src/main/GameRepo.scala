...
object GameRepo {
  ...
  def recentPovsByUser(user: User, nb: Int): Fu[List[Pov]] =
    coll.find(Query user user).sort(Query.sortCreated).cursor[Game]().gather[List](nb).map {
      _.flatMap(g => Pov(g, user))
    }

  def gamesForAssessment(userId: String, nb: Int): Fu[List[Game]] =
    coll
      .find(Query.finished ++ Query.rated ++ Query.user(userId) ++ Query.analysed(true) ++ Query
          .turnsMoreThan(20) ++ Query.clock(true))
  ...
  def setRatingDiffs(id: ID, white: Int, black: Int) =
    coll.update($id(id),
                $set(s"${F.whitePlayer}.${Player.BSONFields.ratingDiff}" -> white,
                     s"${F.blackPlayer}.${Player.BSONFields.ratingDiff}" -> black))

  // used by RatingFest
  def setRatingAndDiffs(id: ID, white: (Int, Int), black: (Int, Int)) =
    coll.update($id(id),
                $set(s"${F.whitePlayer}.${Player.BSONFields.rating}" -> white._1,
  ...
  def lastPlayedPlaying(user: User): Fu[Option[Pov]] =
    coll.find(Query recentlyPlaying user.id).sort(Query.sortUpdatedNoIndex).uno[Game].map {
  ...
  def lastFinishedRatedNotFromPosition(user: User): Fu[Option[Game]] =
    coll
      .find(
        Query.user(user.id) ++ Query.rated ++ Query.finished ++ Query.turnsMoreThan(2) ++ Query.notFromPosition
      )
  ...
  def onTv(nb: Int): Fu[List[Game]] =
    coll.find($doc(F.tvAt $exists true)).sort($sort desc F.tvAt).cursor[Game]().gather[List](nb)
  ...
  def setHoldAlert(pov: Pov, mean: Int, sd: Int, ply: Option[Int] = None) = {
    ...
    coll
      .update(
        $id(pov.gameId),
        $set(
          s"p${pov.color.fold(0, 1)}.${Player.BSONFields.holdAlert}" -> Player.HoldAlert(
            ply = ply | pov.game.turns, mean = mean, sd = sd)
        )
      )
      .void
  }
  ...
  def finish(id: ID, winnerColor: Option[Color], winnerId: Option[String], status: Status) = {
    val partialUnsets = $doc(F.positionHashes -> true,
                             F.playingUids -> true,
  ...
  def insertDenormalized(
      g: Game, ratedCheck: Boolean = true, initialFen: Option[chess.format.FEN] = None): Funit = {
    val g2 =
      if (ratedCheck && g.rated && g.userIds.distinct.size != 2) g.copy(mode = chess.Mode.Casual)
      else g
    val userIds = g2.userIds.distinct
    val fen =
      initialFen.map(_.value) orElse {
        (!g2.variant.standardInitialPosition)
          .option(Forsyth >> g2.toChess)
          .filter(Forsyth.initial !=)
      }
    val bson =
      (gameBSONHandler write g2) ++ $doc(
        F.initialFen -> fen,
  ...
  def removeRecentChallengesOf(userId: String) =
    coll.remove(Query.created ++ Query.friend ++ Query.user(userId) ++ Query.createdSince(
  ...
  def saveNext(game: Game, nextId: ID): Funit =
    coll
      .update(
        $id(game.id),
        $set(F.next -> nextId) ++ $unset("p0." + Player.BSONFields.isOfferingRematch,
  ...
  def bestOpponents(userId: String, limit: Int): Fu[List[(String, Int)]] = {
    ...
    coll
      .aggregate(Match($doc(F.playerUids -> userId)),
                 List(Match($doc(F.playerUids -> $doc("$size" -> 2))),
  ...
  def random: Fu[Option[Game]] =
    coll.find($empty).sort(Query.sortCreated).skip(Random nextInt 1000).uno[Game]
