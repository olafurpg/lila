...
object GameRepo {
  ...
  def lastPlayedPlaying(user: User): Fu[Option[Pov]] =
    coll
      .find(Query.recentlyPlaying(user.id))
  ...
  def insertDenormalized(
      g: Game,
      ratedCheck: Boolean = true,
      initialFen: Option[chess.format.FEN] = None): Funit = {
    val g2 =
  ...
  def getUserIds(id: ID): Fu[List[String]] =
    coll
      ...
      .uno[Bdoc]
      .map { ~_.flatMap(_.getAs[List[String]](F.playerUids)) }
