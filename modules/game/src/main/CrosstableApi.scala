...
final class CrosstableApi(coll: Coll, gameColl: Coll) {
  ...
  def apply(u1: String, u2: String): Fu[Option[Crosstable]] =
    coll.find(select(u1, u2)).uno[Crosstable] orElse create(u1, u2) recoverWith lila.db
      .recoverDuplicateKey(_ => coll.find(select(u1, u2)).uno[Crosstable])
    ...
    case List(u1, u2) =>
      ...
      val bson =
        $doc(
          ...
          "$push" -> $doc(Crosstable.BSONFields.results -> $doc(
  ...
  private def create(x1: String, x2: String): Fu[Option[Crosstable]] =
    UserRepo.orderByGameCount(x1, x2) map (_ -> List(x1, x2).sorted) flatMap {
      case (Some((u1, u2)), List(su1, su2)) =>
        val selector = $doc(Game.BSONFields.playerUids -> $doc("$all" -> List(u1, u2)),
        ...
        for {
          ...
          ctDraft = Crosstable(
            Crosstable.User(su1, 0), Crosstable.User(su2, 0), localResults, nbGames)

          crosstable <- gameColl
            .aggregate(
              Match(selector), List(GroupField(Game.BSONFields.winnerId)("nb" -> SumValue(1))))
            .map(
