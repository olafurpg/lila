...
object Event {
  ...
  private def withCrazyData(data: Option[Crazyhouse.Data], drops: Option[List[Pos]])(
  ...
  case class Move(orig: Pos,
    ...
    def data =
      MoveOrDrop
        .data(fen, check, threefold, state, clock, possibleMoves, possibleDrops, crazyData) {
  ...
  case class Drop(role: chess.Role,
    ...
    def data =
      MoveOrDrop
        .data(fen, check, threefold, state, clock, possibleMoves, possibleDrops, crazyData) {
        Json.obj("role" -> role.name, "uci" -> s"${role.pgn}@${pos.key}", "san" -> san)
      }
  }
  object Drop {
  ...
  case class RedirectOwner(color: Color, id: String, cookie: Option[JsObject]) extends Event {
    def typ = "redirect"
    def data =
      Json
        .obj(
  ...
  case class Clock(white: Float, black: Float) extends Event {
    def typ = "clock"
    def data = Json.obj("white" -> truncateAt(white, 2), "black" -> truncateAt(black, 2))
  }
  object Clock {
  ...
  case class Crowd(white: Boolean, black: Boolean, watchers: JsValue) extends Event {
    def typ = "crowd"
    def data = Json.obj("white" -> white, "black" -> black, "watchers" -> watchers)
  }
