...
case class Pov(game: Game, color: Color) {
  ...
  lazy val remainingSeconds: Option[Int] = game.clock.map(_.remainingTime(color).toInt).orElse {
...
object Pov {
  ...
  def first(game: Game) =
    apply(game, if (!game.variant.racingKings) game.firstPlayer else game.whitePlayer)
  def second(game: Game) =
    apply(game, if (!game.variant.racingKings) game.secondPlayer else game.blackPlayer)
