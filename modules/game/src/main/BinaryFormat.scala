...
object BinaryFormat {
  ...
  object moveTime {
    ...
    private val encodeList: List[(MT, Int)] = List(
      1, 5, 10, 15, 20, 30, 40, 50, 60, 80, 100, 150, 200, 300, 400, 600).zipWithIndex
  ...
  case class clock(since: DateTime) {
    ...
    def write(clock: Clock): ByteArray = ByteArray {
      ...
      Array(writeClockLimit(clock.limit), writeInt8(clock.increment)) ++ time(clock.whiteTime) ++ time(
        clock.blackTime) ++ timer(clock.timerOption getOrElse 0d) map (_.toByte)
    }
