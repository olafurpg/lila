...
final class PgnDump(netBaseUrl: String, getLightUser: String => Option[LightUser]) {
  ...
  def apply(game: Game, initialFen: Option[String]): Pgn = {
    ...
    val moves2 =
      fenSituation.??(_.situation.color.black).fold(".." :: game.pgnMoves, game.pgnMoves)
  ...
  private val customStartPosition: Set[chess.variant.Variant] = Set(chess.variant.Chess960,
                                                                    chess.variant.FromPosition,
                                                                    chess.variant.Horde,
                                                                    chess.variant.RacingKings)

  private def tags(
      game: Game, initialFen: Option[String], imported: Option[ParsedPgn]): List[Tag] =
  ...
  private def turns(moves: List[String], from: Int): List[chessPgn.Turn] =
    (moves grouped 2).zipWithIndex.toList map {
      case (moves, index) =>
        chessPgn.Turn(number = index + from, white = moves.headOption filter (".." !=) map {
        chessPgn.Move(_)
