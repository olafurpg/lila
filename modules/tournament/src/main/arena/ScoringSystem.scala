...
object ScoringSystem extends AbstractScoringSystem {
  ...
  case class Score(win: Option[Boolean], flag: Flag, berserk: Int) extends AbstractScore {
    ...
    pairings.zip(nexts).foldLeft(List[Score]()) {
      case (scores, (p, n)) =>
        ...
        (p.winner match {
          ...
          case None => Score(None, if (firstTwoAreWins(scores)) Double else Normal, berserkValue)
