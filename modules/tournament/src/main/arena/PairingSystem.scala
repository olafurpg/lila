...
object PairingSystem extends AbstractPairingSystem {
  ...
  def createPairings(tour: Tournament, users: WaitingUsers, ranking: Ranking): Fu[Pairings] = {
    for {
      ...
      onlyTwoActivePlayers <- (tour.nbPlayers > 20).fold(fuccess(false),
  ...
  private def smartPairings(data: Data, players: RankedPlayers): List[Pairing.Prep] =
    players.nonEmpty ?? {
      ...
      def score(pairs: Combination): Score = {
        ...
        pairs.foreach {
          case (a, b) =>
            ...
            Math.abs(a.player.rating - b.player.rating) + justPlayedTogether(a.player.userId,
                                                                             b.player.userId).?? {
      ...
      if (!continue)
        pairingLogger.info(s"smartPairings cutoff! [${nowMillis -
      startAt}ms] ${url(data.tour.id)} ${players.size} players, ${preps.size} preps")
      preps
    }
