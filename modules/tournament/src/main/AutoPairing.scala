...
final class AutoPairing(roundMap: ActorRef, system: ActorSystem, onStart: String => Unit) {
    ...
    for {
      ...
      game1 = Game.make(
        game = chess.Game(
          ...
          g.copy(clock = tour.clock.chessClock.some, turns = turns, startedAtTurn = turns)
        },
