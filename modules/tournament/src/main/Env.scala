...
final class Env(config: Config,
  ...
  lazy val cached = new Cached(createdTtl = CreatedCacheTtl, rankingTtl = RankingCacheTtl)
  ...
  lazy val socketHandler = new SocketHandler(
    hub = hub, socketHub = socketHub, chat = hub.actor.chat, flood = flood)

  lazy val winners = new Winners(mongoCache = mongoCache, ttl = LeaderboardCacheTtl)

  lazy val jsonView = new JsonView(lightUser, cached, performance)

  lazy val scheduleJsonView = new ScheduleJsonView(lightUser)

  lazy val leaderboardApi = new LeaderboardApi(coll = leaderboardColl, maxPerPage = 20)
...
new Sequencer(receiveTimeout = SequencerTimeout.some,
  ...
  system.lilaBus.subscribe(system.actorOf(Props(new ApiActor(api = api)), name = ApiActorName),
                           'finishGame,
                           'adjustCheater,
                           'adjustBooster)
...
object Env {
  ...
  lazy val current =
    "tournament" boot new Env(config = lila.common.PlayApp loadConfig "tournament",
