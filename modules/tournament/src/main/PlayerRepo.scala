...
object PlayerRepo {
  ...
  def join(tourId: String, user: User, perfLens: Perfs => Perf) =
    find(tourId, user.id) flatMap {
      case Some(p) if p.withdraw =>
        coll.update(selectId(p._id), BSONDocument("$unset" -> BSONDocument("w" -> true)))
      case Some(p) => funit
  ...
  def withdraw(tourId: String, userId: String) =
    coll
      .update(selectTourUser(tourId, userId), BSONDocument("$set" -> BSONDocument("w" -> true)))
      .void
  ...
  def byTourAndUserIds(tourId: String, userIds: Iterable[String]): Fu[List[Player]] =
    coll
      .find(selectTour(tourId) ++ $doc("uid" -> $doc("$in" -> userIds)))
      .list[Player]()
      .chronometer
  ...
  def rankedByTourAndUserIds(
      tourId: String, userIds: Iterable[String], ranking: Ranking): Fu[RankedPlayers] =
