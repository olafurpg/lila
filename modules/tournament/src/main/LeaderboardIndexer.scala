...
private final class LeaderboardIndexer(tournamentColl: Coll, leaderboardColl: Coll) {
  ...
  def generateAll: Funit =
    leaderboardColl.remove(BSONDocument()) >> {
      tournamentColl
        ...
        .mapM[Tournament]
        .apply[Seq[Entry]](generateTour) &> Enumeratee.mapConcat[Seq[Entry]].apply[Entry](identity) &> Enumeratee
        .grouped(Iteratee takeUpTo 500) |>>> Iteratee.foldM[Seq[Entry], Int](0) {
        case (number, entries) =>
          if (number % 10000 == 0) logger.info(s"Generating leaderboards... $number")
