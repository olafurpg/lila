...
object TournamentRepo {
  ...
  private val enterableSelect = $doc("status" $in (Status.Created.id, Status.Started.id))
  ...
  def byIds(ids: Iterable[String]): Fu[List[Tournament]] =
    coll.find($inIds(ids)).cursor[Tournament]().gather[List]()
  ...
  def recentAndNext: Fu[List[Tournament]] =
    coll.find(sinceSelect(DateTime.now minusDays 1)).cursor[Tournament]().gather[List]()
  ...
  def createdIncludingScheduled: Fu[List[Tournament]] =
    coll.find(createdSelect).list[Tournament](None)
  ...
  def publicStarted: Fu[List[Tournament]] =
    coll
      .find(startedSelect ++ $doc("private" -> $doc("$exists" -> false)))
      .sort($doc("createdAt" -> -1))
      .list[Tournament]()

  def finished(limit: Int): Fu[List[Tournament]] =
    coll.find(finishedSelect).sort($doc("startsAt" -> -1)).list[Tournament](limit)

  def finishedNotable(limit: Int): Fu[List[Tournament]] =
    coll
      .find(finishedSelect ++ $doc("$or" -> $arr(
  ...
  def finishedPaginator(maxPerPage: Int, page: Int) =
    Paginator(adapter = new Adapter[Tournament](
                  collection = coll,
  ...
  def scheduledUnfinished: Fu[List[Tournament]] =
    coll.find(scheduledSelect ++ unfinishedSelect).sort($doc("startsAt" -> 1)).list[Tournament]()

  def scheduledCreated: Fu[List[Tournament]] =
    coll.find(createdSelect ++ scheduledSelect).sort($doc("startsAt" -> 1)).list[Tournament]()
  ...
  def lastFinishedDaily(variant: Variant): Fu[Option[Tournament]] =
    coll
      .find(
        finishedSelect ++ sinceSelect(DateTime.now minusDays 1) ++ variantSelect(variant) ++ $doc(
  ...
  def toursToWithdrawWhenEntering(tourId: String): Fu[List[Tournament]] =
    coll
      .find(enterableSelect ++ $doc(
          "_id" -> $doc("$ne" -> tourId),
          "schedule.freq" $nin (Schedule.Freq.Marathon.name, Schedule.Freq.Unique.name)
        ) ++ nonEmptySelect)
      .cursor[Tournament]()
      .gather[List]()
}
