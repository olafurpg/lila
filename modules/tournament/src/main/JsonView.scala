...
final class JsonView(
    getLightUser: String => Option[LightUser], cached: Cached, performance: Performance) {

  import JsonView._

  private case class CachableData(
      pairings: JsArray, featured: Option[JsObject], podium: Option[JsArray])
        ...
        GameRepo game pairing.gameId flatMap {
          _ ?? { game =>
            cached ranking tour flatMap { ranking =>
              PlayerRepo.pairByTourAndUserIds(tour.id, pairing.user1, pairing.user2) map {
                pairOption =>
                  for {
                    players <- pairOption
                    (p1, p2) = players
  ...
  private val firstPageCache = lila.memo.AsyncCache[String, JsObject](
    (id: String) =>
      TournamentRepo byId id flatten s"No such tournament: $id" flatMap { computeStanding(_, 1) },
    timeToLive = 1 second)
  ...
  private val cachableData = lila.memo.AsyncCache[String, CachableData](
    id =>
      ...
      } yield CachableData(JsArray(pairings map pairingJson), featured map featuredJson, podium),
    timeToLive = 1 second)
  ...
  private def myInfoJson(i: PlayerInfo) = Json.obj("rank" -> i.rank, "withdraw" -> i.withdraw)
  ...
  private def playerJson(
      sheet: Option[ScoreSheet], tour: Tournament, rankedPlayer: RankedPlayer): JsObject = {
            ...
            case rp @ RankedPlayer(_, player) =>
              for {
                ...
                playerJson(sheet.some, tour, rp) ++ Json.obj(
                  "nb" -> sheetNbs(player.userId, sheet, pairings), "performance" -> tpr)
          }.sequenceFu
        } map { l =>
          JsArray(l).some
        }
