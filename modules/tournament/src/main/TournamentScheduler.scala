      ...
      try {
        ...
        val nextSchedules: List[Schedule] = List(
          List( // monthly tournaments!
               at(lastSundayOfCurrentMonth, 16) map { date =>
                 Schedule(Monthly, Bullet, Standard, std, date)
               },
               at(lastSundayOfCurrentMonth, 17) map { date =>
                 Schedule(Monthly, SuperBlitz, Standard, std, date)
               },
               at(lastSundayOfCurrentMonth, 18) map { date =>
                 Schedule(Monthly, Blitz, Standard, std, date)
               },
               at(lastSundayOfCurrentMonth, 19) map { date =>
                 Schedule(Monthly, Classical, Standard, std, date)
               },
               at(lastSundayOfCurrentMonth, 20) map { date =>
                 Schedule(Monthly, Blitz, Crazyhouse, std, date)
               }).flatten,
          List( // weekly tournaments!
               at(nextSaturday, 16) map { date =>
                 Schedule(Weekly, Bullet, Standard, std, date |> orNextWeek)
               },
               at(nextSaturday, 17) map { date =>
                 Schedule(Weekly, SuperBlitz, Standard, std, date |> orNextWeek)
               },
               at(nextSaturday, 18) map { date =>
                 Schedule(Weekly, Blitz, Standard, std, date |> orNextWeek)
               },
               at(nextSaturday, 19) map { date =>
                 Schedule(Weekly, Classical, Standard, std, date |> orNextWeek)
               },
               at(nextSaturday, 20) map { date =>
                 Schedule(Weekly, Blitz, Crazyhouse, std, date |> orNextWeek)
               }).flatten,
          List( // daily tournaments!
               at(today, 16) map { date =>
                 Schedule(Daily, Bullet, Standard, std, date |> orTomorrow)
               },
               at(today, 17) map { date =>
                 Schedule(Daily, SuperBlitz, Standard, std, date |> orTomorrow)
               },
               at(today, 18) map { date =>
                 Schedule(Daily, Blitz, Standard, std, date |> orTomorrow)
               },
               at(today, 19) map { date =>
                 Schedule(Daily, Classical, Standard, std, date |> orTomorrow)
               },
               at(today, 20) map { date =>
                 Schedule(Daily, HyperBullet, Standard, std, date |> orTomorrow)
               }).flatten,
          List( // daily variant tournaments!
               at(today, 20) map { date =>
                 Schedule(Daily, Blitz, Crazyhouse, std, date |> orTomorrow)
               },
               at(today, 21) map { date =>
                 Schedule(Daily, Blitz, Chess960, std, date |> orTomorrow)
               },
               at(today, 22) map { date =>
                 Schedule(Daily, Blitz, KingOfTheHill, std, date |> orTomorrow)
               },
               at(today, 23) map { date =>
                 Schedule(Daily, Blitz, ThreeCheck, std, date |> orTomorrow)
               },
               at(today, 0) map { date =>
                 Schedule(Daily, Blitz, Antichess, std, date |> orTomorrow)
               },
               at(tomorrow, 1) map { date =>
                 Schedule(Daily, Blitz, Atomic, std, date)
               },
               at(tomorrow, 2) map { date =>
                 Schedule(Daily, Blitz, Horde, std, date)
               },
               at(tomorrow, 3) map { date =>
                 Schedule(Daily, SuperBlitz, RacingKings, std, date)
               }).flatten,
          List( // eastern tournaments!
               at(today, 4) map { date =>
                 Schedule(Eastern, Bullet, Standard, std, date |> orTomorrow)
               },
               at(today, 5) map { date =>
                 Schedule(Eastern, SuperBlitz, Standard, std, date |> orTomorrow)
               },
               at(today, 6) map { date =>
                 Schedule(Eastern, Blitz, Standard, std, date |> orTomorrow)
               },
               at(today, 7) map { date =>
                 Schedule(Eastern, Classical, Standard, std, date |> orTomorrow)
               }).flatten,
            ...
            case (hour, opening) =>
              List(
                at(today, hour) map { date =>
                  Schedule(Hourly, Bullet, Standard, opening, date |> orTomorrow)
                },
                at(today, hour) map { date =>
                  Schedule(Hourly, SuperBlitz, Standard, opening, date |> orTomorrow)
                },
                at(today, hour) map { date =>
                  Schedule(Hourly, Blitz, Standard, opening, date |> orTomorrow)
                },
                at(today, hour) map { date =>
                  Schedule(Hourly, Classical, Standard, opening, date |> orTomorrow)
                }
              ).flatten
          },
          ...
          (0 to 6).toList.flatMap { hourDelta =>
            ...
            List(
              at(date, hour) map { date =>
                Schedule(Hourly, Bullet, Standard, std, date)
              },
              at(date, hour, 30) map { date =>
                Schedule(Hourly, bulletType, Standard, std, date)
              },
              at(date, hour) map { date =>
                Schedule(Hourly, SuperBlitz, Standard, std, date)
              },
              at(date, hour) map { date =>
                Schedule(Hourly, Blitz, Standard, std, date)
              },
              at(date, hour) flatMap { date =>
                (hour % 2 == 0) option Schedule(Hourly, Classical, Standard, std, date)
              }
            ).flatten
          },
          ...
          (0 to 6).toList.flatMap { hourDelta =>
            ...
            List(
              at(date, hour) map { date =>
                Schedule(Hourly, speed, Crazyhouse, std, date)
              },
              at(date, hour, 30) flatMap { date =>
                (speed == Bullet) option Schedule(Hourly, speed, Crazyhouse, std, date)
              }
            ).flatten
          }
        ).flatten
