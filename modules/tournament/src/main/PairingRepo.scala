...
object PairingRepo {
  ...
  private def selectTourUser(tourId: String, userId: String) = $doc("tid" -> tourId, "u" -> userId)
  ...
  def lastOpponents(
      tourId: String, userIds: Iterable[String], nb: Int): Fu[Pairing.LastOpponents] =
    coll
      .find(
      ...
      .sort(recentSort)
      .cursor[Bdoc]()
      .enumerate(nb) |>>> Iteratee.fold(scala.collection.immutable.Map.empty[String, String]) {
      (acc, doc) =>
    ...
    if (g.aborted) coll.remove(selectId(g.id))
      ...
      coll
        .update(
          selectId(g.id),
          $doc(
            "$set" -> $doc("s" -> g.status.id, "w" -> g.winnerColor.map(_.white), "t" -> g.turns)))
        .void
  ...
  def setBerserk(pairing: Pairing, userId: String, value: Int) =
    (userId match {
      ...
      coll.update(selectId(pairing.id), $doc("$set" -> $doc(field -> value))).void
    }
  ...
  def playingUserIds(tour: Tournament): Fu[Set[String]] =
    coll
      .aggregate(Match(selectTour(tour.id) ++ selectPlaying),
                 List(Project($doc("u" -> BSONBoolean(true), "_id" -> BSONBoolean(false))),
                      Unwind("u"),
                      Group(BSONBoolean(true))("ids" -> AddToSet("u"))))
      .map(
        _.documents.headOption.flatMap(_.getAs[Set[String]]("ids")).getOrElse(Set.empty[String]))
  ...
  def playingGameIds(tourId: String): Fu[List[String]] =
    coll
      ...
      .map(
        _.documents.headOption.flatMap(_.getAs[List[String]]("ids")).getOrElse(List.empty[String]))
}
