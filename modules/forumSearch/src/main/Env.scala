...
final class Env(
    config: Config, postApi: PostApi, makeClient: Index => ESClient, system: ActorSystem) {
  ...
  private lazy val paginatorBuilder =
    new lila.search.PaginatorBuilder(searchApi = api, maxPerPage = PaginatorMaxPerPage)
...
object Env {
  ...
  lazy val current =
    "forumSearch" boot new Env(config = lila.common.PlayApp loadConfig "forumSearch",
