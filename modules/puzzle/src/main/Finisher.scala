...
private[puzzle] final class Finisher(api: PuzzleApi, puzzleColl: Coll) {
  ...
  def apply(
      puzzle: Puzzle, user: User, data: DataForm.AttemptData): Fu[(Attempt, Option[Boolean])] =
    api.attempt.find(puzzle.id, user.id) flatMap {
      ...
      case None =>
        ...
        updateRatings(
          userRating, puzzleRating, data.isWin.fold(Glicko.Result.Win, Glicko.Result.Loss))
        val date = DateTime.now
        val puzzlePerf = puzzle.perf.addOrReset(_.puzzle.crazyGlicko, s"puzzle ${puzzle.id} user")(
          puzzleRating, date)
        val userPerf = user.perfs.puzzle
          .addOrReset(_.puzzle.crazyGlicko, s"puzzle ${puzzle.id}")(userRating, date)
        ...
        ((api.attempt add a) >> {
              puzzleColl.update($id(puzzle.id),
                                $inc(
