...
private[puzzle] final class PuzzleApi(puzzleColl: Coll, attemptColl: Coll, apiToken: String) {
  ...
  object puzzle {
    ...
    def latest(nb: Int): Fu[List[Puzzle]] =
      puzzleColl.find($empty).sort($doc("date" -> -1)).cursor[Puzzle]().gather[List](nb)
    ...
    def insertPuzzles(puzzles: List[Try[PuzzleId => Puzzle]]): Fu[List[Try[PuzzleId]]] =
      puzzles match {
  ...
  object attempt {
    ...
    def vote(a1: Attempt, v: Boolean): Fu[(Puzzle, Attempt)] = puzzle find a1.puzzleId flatMap {
      ...
      case Some(p1) =>
        ...
        attemptColl.update(
          $doc("_id" -> a2.id),
          $doc("$set" -> $doc(Attempt.BSONFields.vote -> v))) zip puzzleColl.update(
          $doc("_id" -> p2.id), $doc("$set" -> $doc(Puzzle.BSONFields.vote -> p2.vote))) map {
          case _ => p2 -> a2
        }
    }
    ...
    def playedIds(user: User, max: Int): Fu[BSONArray] =
      attemptColl.distinct(Attempt.BSONFields.puzzleId,
                           $doc(Attempt.BSONFields.userId -> user.id).some) map BSONArray.apply

    def hasVoted(user: User): Fu[Boolean] =
      attemptColl
        .find($doc(Attempt.BSONFields.userId -> user.id),
              $doc(
