  ...
  object attempt {
    ...
    def vote(a1: Attempt, v: Boolean): Fu[(Puzzle, Attempt)] = puzzle.find(a1.puzzleId).flatMap {
      ...
      case Some(p1) =>
        ...
        attemptColl
          .update($doc("_id" -> a2.id), $doc("$set" -> $doc(Attempt.BSONFields.vote -> v)))
          .zip(puzzleColl
            .update($doc("_id" -> p2.id), $doc("$set" -> $doc(Puzzle.BSONFields.vote -> p2.vote))))
          .map {
            case _ => p2 -> a2
          }
    }
    ...
    def playedIds(user: User, max: Int): Fu[BSONArray] =
      attemptColl
        .distinct(Attempt.BSONFields.puzzleId, $doc(Attempt.BSONFields.userId -> user.id).some)
        .map(BSONArray.apply)
