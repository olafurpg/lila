...
object JsonApi {
  ...
  object Request {
    ...
    case class Fishnet(version: Client.Version, python: Option[Client.Python], apikey: Client.Key)
    ...
    case class PostAnalysis(fishnet: Fishnet, engine: FullEngine, analysis: List[Evaluation])
        extends Request
        with Result {
  ...
  case class Game(game_id: String, position: FEN, variant: Variant, moves: String)
