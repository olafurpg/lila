...
private final class FishnetRepo(analysisColl: Coll, clientColl: Coll) {
  ...
  private val clientCache = AsyncCache[Client.Key, Option[Client]](
    f = key => clientColl.find(selectClient(key)).uno[Client], timeToLive = 10 seconds)
  ...
  def updateClient(client: Client): Funit =
    clientColl.update(selectClient(client.key), client, upsert = true).void >> clientCache.remove(
      client.key)
  ...
  def enableClient(key: Client.Key, v: Boolean): Funit =
    clientColl.update(selectClient(key), BSONDocument("$set" -> BSONDocument("enabled" -> v))).void >> clientCache
      .remove(key)
  ...
  def getAnalysisByGameId(gameId: String) =
    analysisColl
      .find(BSONDocument(
          "game.id" -> gameId
        ))
