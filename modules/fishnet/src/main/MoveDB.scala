...
private final class MoveDB(roundMap: ActorSelection, system: ActorSystem) {
  ...
  private case class PostResult(moveId: Work.Id,
                                client: Client,
                                data: JsonApi.Request.PostMove,
                                measurement: lila.mon.Measurement)
      ...
      case Some(move) if move isAcquiredBy client =>
        data.move.uci match {
          case Some(uci) =>
            ...
            roundMap ! hubApi.map.Tell(move.game.id,
                                       hubApi.round.FishnetPlay(uci, move.currentFen))
          case _ =>
