...
final class Cached(userColl: Coll,
  ...
  private val countCache = mongoCache.single[Int](
    prefix = "user:nb", f = userColl.count(UserRepo.enabledSelect.some), timeToLive = nbTtl)
  ...
  val top50Online = lila.memo.AsyncCache.single[List[User]](
    f = UserRepo.byIdsSortRating(onlineUserIdMemo.keys, 50), timeToLive = 10 seconds)
