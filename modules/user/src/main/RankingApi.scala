...
final class RankingApi(coll: Coll,
  ...
  object weeklyStableRanking {
    ...
    private val cache =
      AsyncCache[Perf.ID, Map[User.ID, Rank]](f = compute, timeToLive = 15 minutes)
  ...
  object weeklyRatingDistribution {
    ...
    private val cache = mongoCache[Perf.ID, List[NbUsers]](prefix = "user:rating:distribution",
                                                           f = compute,
                                                           timeToLive = 3 hour,
    ...
    private def compute(perfId: Perf.ID): Fu[List[NbUsers]] =
      lila.rating.PerfType(perfId).exists(lila.rating.PerfType.leaderboardable.contains) ?? {
        coll
          .aggregate(Match($doc("perf" -> perfId)),
                     List(Project(
                            $doc(
                              "_id" -> false,
                              "r" -> $doc(
