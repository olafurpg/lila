...
final class Api(coll: Coll,
  ...
  def inbox(me: User, page: Int): Fu[Paginator[Thread]] = Paginator(
    adapter = new Adapter(collection = coll,
    ...
    UserRepo named data.user.id flatMap {
      _.fold(fufail[Thread]("No such recipient")) { invited =>
        Thread.make(name = data.subject,
          ...
          val thread =
            if (me.troll || lila.security.Spam.detect(data.subject, data.text)) t deleteFor invited
            else t
  ...
  def lichessThread(lt: LichessThread): Funit =
    sendUnlessBlocked(
      Thread.make(name = lt.subject, text = lt.message, creatorId = lt.from, invitedId = lt.to),
  ...
  def deleteThread(id: String, me: User): Funit =
    thread(id, me) flatMap { threadOption =>
      (threadOption.map(_.id) ?? (ThreadRepo deleteFor me.id)) >>- updateUser(me)
    }
  ...
  def updateUser(user: lila.user.User) {
    if (!user.kid)
      (unreadCache refresh user.id) mapTo manifest[List[String]] foreach { ids =>
