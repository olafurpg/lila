...
final class MongoCache[K, V : MongoCache.Handler] private (prefix: String,
                                                           expiresAt: () => DateTime,
                                                           cache: Cache[V],
                                                           coll: Coll,
                                                           f: K => Fu[V],
  ...
  def apply(k: K): Fu[V] = cache(k) {
    coll.find(select(k)).uno[Entry] flatMap {
      case None =>
        f(k) flatMap { v =>
          coll.insert(makeEntry(k, v)) recover lila.db.recoverDuplicateKey(_ => ()) inject v
        }
...
object MongoCache {
  ...
  final class Builder(coll: Coll) {
    ...
    def apply[K, V : Handler](prefix: String,
                              ...
                              keyToString: K => String): MongoCache[K, V] =
      new MongoCache[K, V](prefix = prefix,
