...
object AsyncCache {
  ...
  def apply[K, V](f: K => Fu[V],
    ...
    new AsyncCache(cache = LruCache(maxCapacity, initialCapacity, timeToLive, timeToIdle), f = f)

  def single[V](f: => Fu[V], timeToLive: Duration = Duration.Inf) =
    new AsyncCache[Boolean, V](cache = LruCache(timeToLive = timeToLive), f = _ => f)
}
