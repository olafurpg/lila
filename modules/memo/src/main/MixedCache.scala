...
object MixedCache {
  ...
  def apply[K, V](
    ...
    val sync = Builder.cache[K, V](timeToLive, (k: K) => async(k).await(makeTimeout(awaitTime)))
  ...
  def single[V](
    ...
    val sync = Builder
      .cache[Boolean, V](timeToLive, (_: Boolean) => async(true).await(makeTimeout(awaitTime)))
