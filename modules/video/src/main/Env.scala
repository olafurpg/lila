...
final class Env(config: Config, scheduler: lila.common.Scheduler, db: lila.db.Env, isDev: Boolean) {
  ...
  private lazy val youtube =
    new Youtube(url = YoutubeUrl, apiKey = YoutubeApiKey, max = YoutubeMax, api = api)
