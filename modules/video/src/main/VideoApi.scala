...
private[video] final class VideoApi(videoColl: Coll, viewColl: Coll) {
  ...
  object video {
    ...
    def search(user: Option[User], query: String, page: Int): Fu[Paginator[VideoView]] = {
      ...
      Paginator(adapter = new Adapter[Video](
    ...
    def save(video: Video): Funit =
      videoColl.update($doc("_id" -> video.id), $doc("$set" -> video), upsert = true).void
    ...
    object count {
      ...
      private val cache = AsyncCache.single(f = videoColl.count(none), timeToLive = 1.day)
  ...
  object view {
    ...
    def add(a: View) = (viewColl insert a).void recover lila.db.recoverDuplicateKey(_ => ())
    ...
    def seenVideoIds(user: User, videos: Seq[Video]): Fu[Set[Video.ID]] =
      viewColl.distinct(View.BSONFields.videoId, $inIds(videos.map { v =>
      ...
      f = filterTags =>
          ...
          val allPaths =
            if (filterTags.isEmpty)
              ...
              videoColl
                .aggregate(Match($doc("tags".$all(filterTags: _*))),
    ...
    private val popularCache = AsyncCache.single[List[TagNb]](
      f = videoColl
          .aggregate(
            ...
            List(Unwind("tags"), GroupField("tags")("nb" -> SumValue(1)), Sort(Descending("nb"))))
          .map(_.documents.flatMap(_.asOpt[TagNb])),
      timeToLive = 1.day)
  }
}
