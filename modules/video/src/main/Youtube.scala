...
private[video] final class Youtube(url: String, apiKey: String, max: Int, api: VideoApi) {
  ...
  private implicit val readEntries: Reads[Seq[Entry]] = (__ \ "items").read(Reads seq readEntry)
...
object Youtube {
  ...
  private[video] case class Entry(
      id: String, snippet: Snippet, statistics: Statistics, contentDetails: ContentDetails)

  private[video] case class Snippet(description: Option[String], publishedAt: Option[String])

  private[video] case class Statistics(viewCount: String, likeCount: String, dislikeCount: String)
