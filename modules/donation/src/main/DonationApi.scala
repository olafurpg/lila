...
final class DonationApi(coll: Coll,
  ...
  def list(nb: Int) =
    coll.find(decentAmount).sort(BSONDocument("date" -> -1)).cursor[Donation]().gather[List](nb)

  def top(nb: Int): Fu[List[lila.user.User.ID]] =
    coll
      .aggregate(Match(BSONDocument("userId" -> BSONDocument("$exists" -> true))),
  ...
  def create(donation: Donation) = {
    coll insert donation recover lila.db.recoverDuplicateKey(e => println(e.getMessage)) void
  } >> progressCache.clear >> donation.userId.??(donorCache.remove) >>- progress.foreach { prog =>
    bus.publish(
      lila.hub.actorApi.DonationEvent(userId = donation.userId,
  ...
  private val progressCache =
    lila.memo.AsyncCache.single[Progress](computeProgress, timeToLive = 10 seconds)
  ...
  private def computeProgress: Fu[Progress] = {
    val from = Try {
      ...
      val a =
        now withDayOfWeek DateTimeConstants.SUNDAY withHourOfDay 0 withMinuteOfHour 0 withSecondOfMinute 0
    ...
    coll
      ...
      .cursor[BSONDocument]()
      .gather[List]() map2 { (obj: BSONDocument) =>
      ~obj.getAs[Int]("net")
