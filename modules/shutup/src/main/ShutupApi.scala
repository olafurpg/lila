...
final class ShutupApi(
    coll: Coll, follows: (String, String) => Fu[Boolean], reporter: akka.actor.ActorSelection) {
  ...
  def getPublicLines(userId: String): Fu[List[String]] =
    coll.find($doc("_id" -> userId), $doc("pub" -> 1)).uno[Bdoc].map {
  ...
  private def record(
      userId: String, text: String, textType: TextType, toUserId: Option[String] = None): Funit =
          ...
          case false =>
            ...
            val pushPublicLine =
              if (textType == TextType.PublicChat && analysed.nbBadWords > 0)
                $doc(
                  "pub" -> $doc("$each" -> List(text), "$slice" -> -20)
                )
              else $empty
            val push =
              $doc(
                textType.key -> $doc("$each" -> List(BSONDouble(analysed.ratio)),
                                     "$slice" -> -textType.rotation)
              ) ++ pushPublicLine
            coll
              .findAndUpdate(selector = $doc("_id" -> userId),
    ...
    userRecord.reports.exists(_.unacceptable) ?? {
      ...
      coll
        .update(
          ...
          $doc("$unset" -> $doc(TextType.PublicForumMessage.key -> true,
