...
case class Seek(
  ...
  private def ratingRangeCompatibleWith(h: Seek) = realRatingRange.fold(true) { range =>
  ...
  def render: JsObject = Json.obj(
    ...
    "rating" -> rating,
    "variant" -> Json
      .obj("key" -> realVariant.key, "short" -> realVariant.shortName, "name" -> realVariant.name),
    "mode" -> realMode.id,
    "days" -> daysPerTurn,
    "color" -> chess.Color(color).??(_.name),
    "perf" -> Json.obj("icon" -> perfType.map(_.iconChar.toString), "name" -> perfType.map(_.name))
  )
