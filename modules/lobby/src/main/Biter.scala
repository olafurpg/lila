...
private[lobby] object Biter {
  ...
  private def assignCreatorColor(
      creatorUser: Option[User], joinerUser: Option[User], color: Color): Fu[chess.Color] =
    color match {
      case Color.Random =>
  ...
  private def makeGame(hook: Hook) =
    Game.make(game = ChessGame(board = Board init hook.realVariant, clock = hook.clock.some),
  ...
  private def makeGame(seek: Seek) =
    Game.make(game = ChessGame(board = Board init seek.realVariant, clock = none),
              whitePlayer = Player.white,
              blackPlayer = Player.black,
              mode = seek.realMode,
  ...
  def canJoin(hook: Hook, user: Option[LobbyUser]): Boolean =
    ...
    ) && !(hook.userId ?? (user ?? (_.blocking)).contains) && !((user map (_.id)) ??
        (hook.user ?? (_.blocking)).contains) && hook.realRatingRange.fold(true) { range =>
      user ?? { u =>
  ...
  def canJoin(seek: Seek, user: LobbyUser): Boolean =
    ...
    !(user.blocking contains seek.user.id) && !(seek.user.blocking contains user.id) &&
