...
final class Env(
  ...
  private val socket = system.actorOf(
    Props(new Socket(history = history, router = hub.actor.router, uidTtl = SocketUidTtl)),
    name = SocketName)
  ...
  val lobby =
    Lobby.start(system, ActorName, broomPeriod = BroomPeriod, resyncIdsPeriod = ResyncIdsPeriod) {
      new Lobby(
        socket = socket,
        seekApi = seekApi,
        blocking = blocking,
  ...
  lazy val socketHandler =
    new SocketHandler(hub = hub, lobby = lobby, socket = socket, blocking = blocking)
