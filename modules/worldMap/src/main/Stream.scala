...
private final class Stream(geoIp: MaxMindIpGeo, geoIpCacheTtl: Duration) extends Actor {
  ...
  def receive = {
    ...
    case Stream.Get =>
      sender ! {
        Enumerator.enumerate(games.values.map(game2json(makeMd5))) andThen Enumerator.enumerate(
          List(loadCompleteJson)) andThen producer
      }
  }
