...
final class PlaybanApi(coll: Coll, isRematch: String => Boolean) {
  ...
  def abort(pov: Pov): Funit =
    blameable(pov.game) ?? {
      if (pov.game.olderThan(45)) pov.game.playerWhoDidNotMove.map { Blame(_, Outcome.NoPlay) } else if (pov.game
          .olderThan(15)) none
    ...
    userId => {
      coll
        .findAndUpdate(
          ...
          update = $doc(
            "$push" -> $doc(
              "o" -> $doc("$each" -> List(outcome), "$slice" -> -20)
            )),
          fetchNewObject = true,
          upsert = true)
        .map(_.value)
