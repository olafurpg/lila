...
final class PaginatorBuilder[A, Q : Writes](searchApi: SearchReadApi[A, Q], maxPerPage: Int) {

  def apply(query: Q, page: Int): Fu[Paginator[A]] =
    Paginator(adapter = new ESAdapter(query), currentPage = page, maxPerPage = maxPerPage)
