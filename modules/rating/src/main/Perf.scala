...
case class Perf(glicko: Glicko, nb: Int, recent: List[Int], latest: Option[DateTime]) {
  ...
  def addOrReset(monitor: lila.mon.IncPath, msg: => String)(r: Rating, date: DateTime): Perf =
    add(r, date) | {
  ...
  def toRating =
    new Rating(math.max(Glicko.minRating, glicko.rating), glicko.deviation, glicko.volatility, nb)
