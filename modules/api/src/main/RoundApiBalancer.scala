...
private[api] final class RoundApiBalancer(system: ActorSystem, api: RoundApi, nbActors: Int) {
        ...
        .props(Props(new lila.hub.SequentialProvider {
          ...
          def process = {
            ...
            case Watcher(
                ...
                withOpening,
                ctx) =>
              api.watcher(pov, apiVersion, tv, analysis, initialFenO, withMoveTimes, withOpening)(
                ctx)
  ...
  def watcher(
    ...
    (router ? Watcher(pov, apiVersion, tv, analysis, initialFenO, withMoveTimes, withOpening, ctx))
