...
private[api] final class GameApi(
    netBaseUrl: String, apiToken: String, pgnDump: PgnDump, analysisApi: AnalysisApi) {
    ...
    Paginator(adapter = new CachedAdapter(
                  adapter = new Adapter[Game](
                      ...
                      selector = $doc(
                          ...
                          G.rated -> rated.map(
                            _.fold[BSONValue](BSONBoolean(true), $doc("$exists" -> false))),
          ...
          p.color.name -> Json
            .obj(
              ...
              "moveTimes" -> withMoveTimes.fold(
                g.moveTimes.zipWithIndex.filter(_._2 % 2 == i).map(_._1), JsNull),
