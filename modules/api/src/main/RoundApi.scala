...
private[api] final class RoundApi(jsonView: JsonView,
  ...
  def player(pov: Pov, apiVersion: Int)(implicit ctx: Context): Fu[JsObject] =
    GameRepo.initialFen(pov.game) flatMap { initialFen =>
      jsonView.playerJson(
        ...
        withBlurs = ctx.me ?? Granter(_.ViewBlurs)) zip getTourAndRanks(pov.game) zip
      (pov.game.simulId ?? getSimul) zip (ctx.me ?? (me => noteApi.get(pov.gameId, me.id))) zip forecastApi
        .loadForDisplay(pov) map {
        case ((((json, tourOption), simulOption), note), forecast) =>
          (blindMode _ compose withTournament(pov, tourOption) _ compose withSimul(pov,
                                                                                   simulOption) _ compose withSteps(
                pov,
                none,
                initialFen,
                withOpening = false) _ compose withNote(note) _ compose withBookmark(
                ctx.me ?? { bookmarkApi.bookmarked(pov.game, _) }) _ compose withForecastCount(
                forecast.map(_.steps.size)) _)(json)
      }
    }
  ...
  def watcher(pov: Pov,
    ...
    initialFenO.fold(GameRepo initialFen pov.game)(fuccess) flatMap { initialFen =>
      ...
      (pov.game.simulId ?? getSimul) zip (ctx.me ?? (me => noteApi.get(pov.gameId, me.id))) map {
        case (((json, tourOption), simulOption), note) =>
          (blindMode _ compose withTournament(pov, tourOption) _ compose withSimul(
                pov, simulOption) _ compose withNote(note) _ compose withBookmark(
                ctx.me ?? { bookmarkApi.bookmarked(pov.game, _) }) _ compose withSteps(
                pov, analysis, initialFen, withOpening = withOpening) _ compose withAnalysis(
                analysis) _)(json)
      }
    }

  def userAnalysisJson(
      pov: Pov, pref: Pref, initialFen: Option[String], orientation: chess.Color, owner: Boolean) =
    owner.??(forecastApi loadForDisplay pov).flatMap { fco =>
      jsonView.userAnalysisJson(pov, pref, orientation, owner = owner) map withSteps(
        pov, none, initialFen, withOpening = true) _ map withForecast(pov, owner, fco) _
    }

  private def withSteps(
      pov: Pov, a: Option[(Pgn, Analysis)], initialFen: Option[String], withOpening: Boolean)(
      obj: JsObject) =
    obj +
    ("steps" -> lila.round.StepBuilder(id = pov.game.id,
