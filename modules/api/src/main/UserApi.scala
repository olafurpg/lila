...
private[api] final class UserApi(jsonView: lila.user.JsonView,
  ...
  def list(teamId: String, nb: Option[Int], engine: Option[Boolean]): Fu[JsObject] =
    ...
    UserRepo named username flatMap {
      ...
      case Some(u) =>
        ...
        (ctx.me.filter(u !=) ?? { me =>
            ...
            }) zip relationApi.countFollowing(u.id) zip relationApi.countFollowers(u.id) zip ctx.isAuth.?? {
          prefApi followable u.id
        } zip ctx.userId.?? { relationApi.fetchRelation(_, u.id) } zip ctx.userId.?? {
