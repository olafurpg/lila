...
object Twitch {
  case class Channel(
      url: Option[String], status: Option[String], name: String, display_name: String)
...
object Hitbox {
  ...
  case class Result(livestream: List[Stream]) {
    def streamsOnAir(streamers: List[Streamer]) = livestream.flatMap { s =>
      for {
        streamer <- StreamerList.findHitbox(streamers)(s.media_user_name) if s.media_is_live == "1"
      } yield
