      ...
      streamers =>
        ...
        val twitch =
          WS.url("https://api.twitch.tv/kraken/streams")
            .withQueryString(
              "channel" -> streamers.filter(_.twitch).map(_.streamerName).mkString(","))
        ...
        val hitbox =
          WS.url("http://api.hitbox.tv/media/live/" +
              streamers.filter(_.hitbox).map(_.streamerName).mkString(","))
            .get() map {
            res =>
