  ...
  def apply(opening: Opening, user: User, win: Boolean): Fu[(Attempt, Option[Boolean])] = {
    api.attempt.find(opening.id, user.id) flatMap {
      ...
      case None =>
        ...
        val userPerf = user.perfs.opening
          .addOrReset(_.opening.crazyGlicko, s"opening ${opening.id} user")(userRating, date)
        val openingPerf = opening.perf.addOrReset(_.opening.crazyGlicko, s"opening ${opening.id}")(
          openingRating, date)
        val a = new Attempt(id = Attempt.makeId(opening.id, user.id),
