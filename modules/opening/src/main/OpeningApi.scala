...
private[opening] final class OpeningApi(
    openingColl: Coll, attemptColl: Coll, nameColl: Coll, apiToken: String) {
  ...
  object attempt {
    ...
    def playedIds(user: User, max: Int): Fu[BSONArray] = {
      ...
      val playedIdsGroup = Group($boolean(true))("ids" -> Push(Attempt.BSONFields.openingId))

      col
        .aggregate(
          Match($doc(Attempt.BSONFields.userId -> user.id)), List(Limit(max), playedIdsGroup))
        .map(_.documents.headOption.flatMap(_.getAs[BSONArray]("ids")).getOrElse(BSONArray()))
    }
  }
