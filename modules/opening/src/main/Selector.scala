...
private[opening] final class Selector(openingColl: Coll,
  ...
  def apply(me: Option[User]): Fu[Opening] =
    (me match {
      case None =>
        openingColl.find($empty).skip(Random nextInt anonSkipMax).uno[Opening] flatten "Can't find a opening for anon player!"
      case Some(user) =>
  ...
  private def tryRange(user: User, tolerance: Int, ids: BSONArray): Fu[Opening] =
    openingColl.uno[Opening]($doc(
        ...
        Opening.BSONFields.rating $gt (user.perfs.opening.intRating - tolerance) $lt
