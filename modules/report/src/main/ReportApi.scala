...
private[report] final class ReportApi(coll: Coll) {
  ...
  private def isAlreadySlain(report: Report, user: User) =
    (report.isCheat && user.engine) || (report.isAutomatic && report.isOther && user.troll) ||
  ...
  def autoProcess(userId: String): Funit =
    coll.update($doc("user" -> userId.toLowerCase), $doc("processedBy" -> "lichess")).void >>- monitorUnprocessed
