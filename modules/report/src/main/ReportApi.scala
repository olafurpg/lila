...
private[report] final class ReportApi(coll: Coll) {
  ...
  def create(setup: ReportSetup, by: User): Funit = !by.troll ?? {
    Reason(setup.reason).fold[Funit](fufail(s"Invalid report reason ${setup.reason}")) { reason =>
      val user = setup.user
      val report =
        Report.make(user = setup.user, reason = reason, text = setup.text, createdBy = by)
  ...
  def autoCheatReport(userId: String, text: String): Funit = {
    ...
    UserRepo.byId(userId).zip(UserRepo.lichess).flatMap {
      case (Some(user), Some(lichess)) =>
        create(
          ReportSetup(user = user, reason = "cheat", text = text, gameId = "", move = ""),
          lichess)
      case _ => funit
    }
  }
  ...
  def autoInsultReport(userId: String, text: String): Funit = {
    UserRepo.byId(userId).zip(UserRepo.lichess).flatMap {
      case (Some(user), Some(lichess)) =>
        create(
          ReportSetup(user = user, reason = "insult", text = text, gameId = "", move = ""),
          lichess)
      case _ => funit
    }
  } >>- monitorUnprocessed
  ...
  def autoProcess(userId: String): Funit =
    coll
      .update($doc("user" -> userId.toLowerCase), $doc("processedBy" -> "lichess"))
      .void >>- monitorUnprocessed
  ...
  private def selectRecent(user: User, reason: Reason) =
    $doc("createdAt".$gt(DateTime.now.minusDays(7)), "user" -> user.id, "reason" -> reason.name)
