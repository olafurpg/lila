...
final class DisposableEmailDomain(providerUrl: String, busOption: Option[lila.common.Bus]) {
  ...
  private def onError(e: Exception) {
    ...
    if (!failed) {
      ...
      busOption.foreach { bus =>
        bus.publish(lila.hub.actorApi.slack
                      .Error(s"Disposable emails list: ${e.getMessage}\nPlease fix $providerUrl"),
                    'slack)
      }
    }
  }
