...
object UserSpy {
  ...
  private[security] def apply(firewall: Firewall, geoIP: GeoIP)(coll: Coll)(
  ...
  private def nextUsers(field: String)(values: Set[Value], user: User)(
      implicit coll: Coll): Fu[Set[User]] =
    values.nonEmpty ?? {
      coll.distinct("user",
                    $doc(
                      field -> $doc("$in" -> values),
