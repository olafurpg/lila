...
object Store {
  ...
  private[security] def save(
      sessionId: String, userId: String, req: RequestHeader, apiVersion: Option[Int]): Funit =
    coll
      .insert(
        $doc(
          "_id" -> sessionId,
  ...
  def delete(sessionId: String): Funit =
    coll.update($doc("_id" -> sessionId), $doc("$set" -> $doc("up" -> false))).void

  def closeUserAndSessionId(userId: String, sessionId: String): Funit =
    coll
      .update($doc("user" -> userId, "_id" -> sessionId, "up" -> true),
              $doc("$set" -> $doc("up" -> false)))
      .void

  def closeUserExceptSessionId(userId: String, sessionId: String): Funit =
    coll
      .update($doc("user" -> userId, "_id" -> $doc("$ne" -> sessionId), "up" -> true),
              $doc("$set" -> $doc("up" -> false)),
              multi = true)
      .void
  ...
  def disconnect(userId: String): Funit =
    coll.update($doc("user" -> userId), $doc("$set" -> $doc("up" -> false)), multi = true).void
