...
final class DataForm(val captcher: akka.actor.ActorSelection, emailAddress: EmailAddress)
  ...
  object signup {
    ...
    val website = Form(
      mapping(
        ...
        "gameId" -> nonEmptyText,
        "move" -> nonEmptyText
      )(SignupData.apply)(_ => None).verifying(captchaFailMessage, validateCaptcha _))
  ...
  val passwordReset = Form(
    mapping(
      ...
      "gameId" -> nonEmptyText,
      "move" -> nonEmptyText
    )(PasswordReset.apply)(_ => None).verifying(captchaFailMessage, validateCaptcha _))
...
object DataForm {
  ...
  case class MobileSignupData(username: String, password: String, email: Option[String])
