...
final class ModApi(logApi: ModlogApi,
  ...
  def troll(mod: String, username: String, value: Boolean): Fu[Boolean] = withUser(username) { u =>
    ...
    changed ?? {
      UserRepo.updateTroll(user).void >>- logApi.troll(mod, user.id, user.troll)
  ...
  def ban(mod: String, username: String): Funit = withUser(username) { user =>
    userSpy(user.id) flatMap { spy =>
      UserRepo.toggleIpBan(user.id) zip logApi.ban(mod, user.id, !user.ipBan) zip user.ipBan.fold(
        firewall unblockIps spy.ipStrings,
        (spy.ipStrings map firewall.blockIp).sequenceFu >> (SecurityStore disconnect user.id)
      ) void
    }
  }
  ...
  def setTitle(mod: String, username: String, title: Option[String]): Funit =
    withUser(username) { user =>
      UserRepo.setTitle(user.id, title) >> lightUserApi.invalidate(user.id) >> logApi.setTitle(
        mod, user.id, title)
    }

  def setEmail(mod: String, username: String, email: String): Funit = withUser(username) { user =>
    UserRepo.email(user.id, email) >> UserRepo.setEmailConfirmed(user.id) >> logApi.setEmail(
      mod, user.id)
  }
