...
final class BoostingApi(
    modApi: ModApi, collBoosting: Coll, nbGamesToMark: Int, ratioGamesToMark: Double) {
  ...
  private val variants = Set[variant.Variant](
    variant.Standard, variant.Chess960, variant.KingOfTheHill, variant.ThreeCheck)
  ...
  def check(game: Game, whiteUser: User, blackUser: User): Funit = {
    if (game.rated && game.accountable && game.playedTurns <= 10 && !game.isTournament &&
        game.winnerColor.isDefined && variants.contains(game.variant) &&
        !game.isCorrespondence && game.clock.fold(false) { _.limitInMinutes >= 1 }) {
