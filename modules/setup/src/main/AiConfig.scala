    ...
    fenGame { chessGame =>
      ...
      Game.make(
        ...
        whitePlayer = Player.make(
            color = ChessColor.White, aiLevel = creatorColor.black option level),
        blackPlayer = Player.make(
            color = ChessColor.Black, aiLevel = creatorColor.white option level),
        mode = Mode.Casual,
        variant = realVariant,
...
object AiConfig extends BaseConfig {
  ...
  def <<(v: Int, tm: Int, t: Double, i: Int, d: Int, level: Int, c: String, fen: Option[String]) =
    new AiConfig(variant = chess.variant.Variant(v) err "Invalid game variant " + v,
