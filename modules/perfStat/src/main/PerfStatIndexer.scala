...
final class PerfStatIndexer(storage: PerfStatStorage, sequencer: ActorRef) {
  ...
  private def compute(user: User, perfType: PerfType): Funit = {
    GameRepo
      .sortedCursor(Query.user(user.id) ++ Query.finished ++ Query.turnsMoreThan(2) ++ Query
                      .variant(PerfType variantOf perfType),
                    Query.sortChronological)
      .enumerate(Int.MaxValue, stopOnError = true) |>>> Iteratee.fold[Game, PerfStat](
      PerfStat.init(user.id, perfType)) {
