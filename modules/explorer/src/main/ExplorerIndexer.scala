...
private final class ExplorerIndexer(gameColl: Coll, endpoint: String, massImportEndpoint: String) {
  ...
  def apply(sinceStr: String): Funit =
    parseDate(sinceStr).fold(fufail[Unit](s"Invalid date $sinceStr")) { since =>
      ...
      val query =
        Query.createdSince(since) ++ Query.rated ++ Query.finished ++ Query.turnsMoreThan(8) ++ Query.noProvisional ++ Query
      ...
      gameColl
        ...
        .enumerate(maxGames, stopOnError = true) &> Enumeratee.mapM[Game].apply[Option[GamePGN]] {
        game =>
          makeFastPgn(game) map {
            _ map { game -> _ }
          }
      } &> Enumeratee.collect { case Some(el) => el } &> Enumeratee.grouped(
        Iteratee takeUpTo batchSize) |>>> Iteratee.foldM[Seq[GamePGN], Long](nowMillis) {
        case (millis, pairs) =>
  ...
  private def makeFastPgn(game: Game): Fu[Option[String]] =
    ~(for {
      ...
      minAverageRating = if (game.variant.exotic) 1520 else 1600 if whiteRating >= minPlayerRating
      if blackRating >= minPlayerRating
      averageRating = (whiteRating + blackRating) / 2 if averageRating >= minAverageRating
      ...
      GameRepo initialFen game flatMap { initialFen =>
        UserRepo.usernamesByIds(game.userIds) map { usernames =>
          ...
          val otherTags = List(s"[LichessID ${game.id}]",
