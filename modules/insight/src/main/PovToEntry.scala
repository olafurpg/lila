...
object PovToEntry {
  ...
  def apply(game: Game, userId: String, provisional: Boolean): Fu[Either[Game, Entry]] =
    enrich(game, userId, provisional).map(_.flatMap(convert).toRight(game)).addFailureEffect { e =>
  ...
  private def enrich(game: Game, userId: String, provisional: Boolean): Fu[Option[RichPov]] =
    if (removeWrongAnalysis(game)) fuccess(none)
      ...
      lila.game.Pov.ofUserId(game, userId) ?? { pov =>
        lila.game.GameRepo
          .initialFen(game)
          .zip(game.metadata.analysed ?? lila.analyse.AnalysisRepo.byId(game.id))
          .map {
            case (fen, an) =>
              for {
                boards <- chess.Replay
                  .boards(moveStrs = game.pgnMoves, initialFen = fen, variant = game.variant)
                  .toOption
                  .flatMap(_.toNel)
