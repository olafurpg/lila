...
case class JsonQuestion(dimension: String, metric: String, filters: Map[String, List[String]]) {
...
object JsonQuestion {
  ...
  def fromQuestion(q: Question[_]) =
    JsonQuestion(dimension = q.dimension.key, metric = q.metric.key, filters = q.filters.map {
