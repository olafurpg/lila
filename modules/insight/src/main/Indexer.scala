...
private final class Indexer(storage: Storage, sequencer: ActorRef) {
  ...
  private def gameQuery(user: User) =
    Query.user(user.id) ++ Query.rated ++ Query.finished ++ Query.turnsMoreThan(2) ++ Query.notFromPosition ++ Query.notHordeOrSincePawnsAreWhite
  ...
  private def fetchFirstGame(user: User): Fu[Option[Game]] =
    if (user.count.rated == 0) fuccess(none)
      ...
      (user.count.rated >= maxGames) ?? GameRepo.coll
        ...
        .skip(maxGames - 1)
        .uno[Game]
    } orElse GameRepo.coll.find(gameQuery(user)).sort(Query.sortChronological).uno[Game]
  ...
  private def computeFrom(user: User, from: DateTime, fromNumber: Int): Funit = {
    storage nbByPerf user.id flatMap { nbs =>
      ...
      GameRepo.sortedCursor(query, Query.sortChronological).enumerate(maxGames, stopOnError = true) &> Enumeratee
        .grouped(Iteratee takeUpTo 4) &> Enumeratee.mapM[Seq[Game]].apply[Seq[Entry]] { games =>
        games.map(toEntry).sequenceFu.map(_.flatten).addFailureEffect { e =>
          println(e)
          e.printStackTrace
        }
      } &> Enumeratee.grouped(Iteratee takeUpTo 50) |>>> Iteratee.foldM[Seq[Seq[Entry]], Int](
        fromNumber) {
        case (number, xs) =>
