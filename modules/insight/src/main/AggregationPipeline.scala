...
private final class AggregationPipeline {
  ...
  private lazy val movetimeIdDispatcher =
    MovetimeRange.reversedNoInf.foldLeft[BSONValue](BSONInteger(MovetimeRange.MTRInf.id)) {
      case (acc, mtr) =>
        $doc(
          "$cond" -> $arr($doc("$lte" -> $arr("$" + F.moves("t"), mtr.tenths.last)), mtr.id, acc))
    }
  ...
  private def groupMulti(d: Dimension[_], metricDbKey: String) =
    Group($doc("dimension" -> dimensionGroupId(d), "metric" -> ("$" + metricDbKey)))(
      "v" -> SumValue(1),
      "ids" -> AddToSet("_id")
    ).some
    ...
    NonEmptyList.nel[PipelineOperator](
      Match(
        selectUserId(userId) ++ gameMatcher ++ (dimension == Dimension.Opening)
          .??($doc(F.eco -> $doc("$exists" -> true))) ++ Metric
      ...
      ((metric match {
            ...
            case M.Movetime =>
              List(
                ...
                sampleMoves,
                group(dimension,
                      GroupFunction("$avg", $doc("$divide" -> $arr("$" + F.moves("t"), 10)))),
                sliceIds
              )
            case M.RatingDiff =>
              List(
