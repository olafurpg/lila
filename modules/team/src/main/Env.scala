...
final class Env(config: Config, hub: lila.hub.Env, db: lila.db.Env) {
  ...
  private[team] lazy val colls = new Colls(
    team = db(CollectionTeam), request = db(CollectionRequest), member = db(CollectionMember))

  lazy val forms = new DataForm(colls.team, hub.actor.captcher)

  lazy val api = new TeamApi(coll = colls,
                             cached = cached,
                             notifier = notifier,
  ...
  lazy val paginator = new PaginatorBuilder(
    coll = colls, maxPerPage = PaginatorMaxPerPage, maxUserPerPage = PaginatorMaxUserPerPage)
  ...
  private lazy val notifier = new Notifier(
    sender = NotifierSender, messenger = hub.actor.messenger, router = hub.actor.router)
}
