...
final class TeamApi(coll: Colls,
                    cached: Cached,
                    notifier: Notifier,
  ...
  def create(setup: TeamSetup, me: User): Option[Fu[Team]] = me.canTeam option {
    ...
    val team = Team.make(name = s.name,
                         ...
                         open = s.isOpen,
                         createdBy = me)
    coll.team.insert(team) >> MemberRepo.add(team.id, me.id) >>- {
  ...
  def update(team: Team, edit: TeamEdit, me: User): Funit = edit.trim |> { e =>
    team.copy(location = e.location, description = e.description, open = e.isOpen) |> { team =>
  ...
  def processRequest(team: Team, request: Request, accept: Boolean): Funit =
    for {
      ...
      _ ← userOption
        .filter(_ => accept)
        .??(user => doJoin(team, user.id) >>- notifier.acceptRequest(team, request))
    } yield ()
  ...
  def delete(team: Team): Funit =
    coll.team.remove($id(team.id)) >> MemberRepo.removeByteam(team.id) >>-
  ...
  def recomputeNbMembers =
    coll.team.find($empty).cursor[Team]().enumerate(Int.MaxValue, stopOnError = true) |>>> Iteratee
      .foldM[Team, Unit](()) {
      case (_, team) =>
