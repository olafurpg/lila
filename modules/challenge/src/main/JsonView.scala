...
final class JsonView(getLightUser: String => Option[lila.common.LightUser]) {
  ...
  def apply(a: AllChallenges): JsObject =
    Json.obj(
      "in" -> a.in.map(apply(Direction.In.some)), "out" -> a.out.map(apply(Direction.Out.some)))

  def show(challenge: Challenge, socketVersion: Int, direction: Option[Direction]) =
    Json.obj("challenge" -> apply(direction)(challenge), "socketVersion" -> socketVersion)
  ...
  private def apply(direction: Option[Direction])(c: Challenge): JsObject = Json.obj(
    ...
    "destUser" -> c.destUser,
    "variant" -> Json.obj(
      "key" -> c.variant.key, "short" -> c.variant.shortName, "name" -> c.variant.name),
    "initialFen" -> c.initialFen,
    "rated" -> c.mode.rated,
    "timeControl" ->
    (c.timeControl match {
          case c @ TimeControl.Clock(l, i) =>
            Json.obj("type" -> "clock", "limit" -> l, "increment" -> i, "show" -> c.show)
    ...
    "perf" -> Json.obj("icon" -> iconChar(c).toString, "name" -> c.perfType.name)
  )
