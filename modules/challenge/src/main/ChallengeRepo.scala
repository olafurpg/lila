...
private final class ChallengeRepo(coll: Coll, maxPerUser: Int) {
  ...
  def like(c: Challenge) =
    ~(for {
      ...
      coll
        .find(selectCreated ++ $doc("challenger.id" -> challengerId, "destUser.id" -> destUserId))
        .uno[Challenge])
  ...
  private[challenge] def realTimeUnseenSince(date: DateTime, max: Int): Fu[List[Challenge]] =
    coll
      .find(selectCreated ++ selectClock ++ $doc(
          "seenAt" -> $doc("$lt" -> date)
        ))
      .cursor[Challenge]()
      .gather[List](max)
  ...
  def setSeenAgain(id: Challenge.ID) =
    coll
      .update(
        $id(id),
        $doc("$set" -> $doc("status" -> Status.Created.id,
                            "seenAt" -> DateTime.now,
  ...
  private def setStatus(
      challenge: Challenge, status: Status, expiresAt: Option[DateTime => DateTime] = None) =
    coll
      .update(
