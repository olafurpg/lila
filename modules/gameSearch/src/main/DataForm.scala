...
private[gameSearch] object DataForm {
  ...
  val DateDelta = """^(\d+)(\w)$""".r
  private val dateConstraint = Constraints.pattern(regex = DateDelta, error = "Invalid date.")
  val dateField = optional(nonEmptyText.verifying(dateConstraint))
}

private[gameSearch] case class SearchData(players: SearchPlayer = SearchPlayer(),
