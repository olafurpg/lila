...
final class Env(config: Config, system: ActorSystem, makeClient: Index => ESClient) {
  ...
  lazy val paginator =
    new PaginatorBuilder[lila.game.Game, Query](searchApi = api, maxPerPage = PaginatorMaxPerPage)

  lazy val forms = new DataForm

  lazy val userGameSearch = new UserGameSearch(forms = forms, paginator = paginator)
...
object Env {
  ...
  lazy val current =
    "gameSearch" boot new Env(config = lila.common.PlayApp loadConfig "gameSearch",
