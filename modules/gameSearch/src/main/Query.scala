...
case class Query(user1: Option[String] = None,
  ...
  def nonEmpty =
    user1.nonEmpty || user2.nonEmpty || winner.nonEmpty || winnerColor.nonEmpty || perf.nonEmpty ||
    source.nonEmpty || status.nonEmpty || turns.nonEmpty || averageRating.nonEmpty ||
    hasAi.nonEmpty || aiLevel.nonEmpty || rated.nonEmpty || date.nonEmpty || duration.nonEmpty ||
    clock.nonEmpty || analysed.nonEmpty
}
...
object Query {
  ...
  val durations = {
    ((30, "30 seconds") :: options(List(60,
                                        ...
                                        60 * 30),
                                   _ / 60,
                                   "%d minute{s}").toList) :+ (60 * 60 * 1, "One hour") :+
    (60 * 60 * 3, "Three hours") :+ (60 * 60 * 24, "One day") :+ (60 * 60 * 24 * 3, "Three days") :+
  ...
  val clockIncs = options(
    List(0, 1, 2, 3, 5, 10, 15, 20, 30, 60, 90, 120, 150, 180), "%d second{s}").toList
  ...
  val turns = options(
    (1 to 5) ++ (10 to 45 by 5) ++ (50 to 90 by 10) ++ (100 to 300 by 25), "%d move{s}")
  ...
  val dates =
    List("0d" -> "Now") ++ options(List(1, 2, 6), "h", "%d hour{s} ago") ++ options(
      1 to 6, "d", "%d day{s} ago") ++ options(1 to 3, "w", "%d week{s} ago") ++ options(
      1 to 6, "m", "%d month{s} ago") ++ options(1 to 5, "y", "%d year{s} ago")
