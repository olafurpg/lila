...
final class DetectLanguage(url: String, key: String) {
  ...
  private case class Detection(language: String, confidence: Float, isReliable: Boolean)
    ...
    WS.url(url)
      ...
      (response.json \ "data" \ "detections").asOpt[List[Detection]] match {
        case None =>
          ...
          None
        case Some(res) =>
          res.filter(_.isReliable).sortBy(-_.confidence).headOption map (_.language) flatMap Lang.get
      }
    } recover {
      case e: Exception =>
...
object DetectLanguage {
  ...
  def apply(config: Config): DetectLanguage =
    new DetectLanguage(url = config getString "api.url", key = config getString "api.key")
}
