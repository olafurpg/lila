...
final class Env(config: Config, scheduler: lila.common.Scheduler, messageApi: lila.message.Api) {
  ...
  lazy val api = new BlogApi(prismicUrl = PrismicApiUrl, collection = PrismicCollection)

  lazy val lastPostCache = new LastPostCache(api, LastPostCacheTtl, PrismicCollection)

  private lazy val notifier =
    new Notifier(blogApi = api, messageApi = messageApi, lichessUserId = NotifySender)
