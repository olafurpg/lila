...
private[blog] final class Notifier(blogApi: BlogApi, messageApi: MessageApi, lichessUserId: String) {

  def sendMessages(prismicId: String): Funit =
    blogApi.prismicApi.flatMap { prismicApi =>
      blogApi.one(prismicApi, none, prismicId).flatten(s"No such document: $prismicId").flatMap {
        post =>
          ThreadRepo
            .visibleByUserContainingExists(user = lichessUserId, containing = post.id)
            .flatMap {
  ...
  private def doSend(post: Document): Funit =
    ThreadRepo.reallyDeleteByCreatorId(lichessUserId) >> {
      ...
      (UserRepo
        .recentlySeenNotKidIdsCursor(DateTime.now.minusWeeks(1))
