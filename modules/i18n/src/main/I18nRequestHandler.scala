...
final class I18nRequestHandler(pool: I18nPool, protocol: String, cdnDomain: String) {

  def apply(req: RequestHeader): Option[Handler] =
    if (HTTPRequest.isRedirectable(req) && req.host != cdnDomain && pool.domainLang(req).isEmpty)
      Some(Action(Redirect(redirectUrl(req))))
    else None

  def forUser(req: RequestHeader, userOption: Option[lila.user.User]): Option[Result] =
    for {
