...
final class DataForm(repo: TranslationRepo,
  ...
  val translation = Form(
    mapping(
      ...
      "gameId" -> nonEmptyText,
      "move" -> nonEmptyText
    )(TransMetadata.apply)(TransMetadata.unapply).verifying(captchaFailMessage, validateCaptcha _))

  def translationWithCaptcha = withCaptcha(translation)

  def process(
      code: String, metadata: TransMetadata, data: Map[String, String], user: String): Funit = {
    val messages =
      (data mapValues { msg =>
    ...
    messages.nonEmpty ?? repo.nextId flatMap { id =>
      ...
      val translation = Translation(_id = id, code = code, text = sorted map {
      case (key, trans) => key + "=" + trans
    } mkString "\n", comment = metadata.comment, author = user.some, createdAt = DateTime.now)
...
private[i18n] case class TransMetadata(comment: Option[String], gameId: String, move: String)
