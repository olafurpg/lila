...
final class I18nKeys(translator: Translator) {
  ...
  val `theOtherPlayerHasLeftTheGameYouCanForceResignationOrWaitForHim` = new Key(
  ...
  val `areYouSureYouEvenRegisteredYourEmailOnLichess` = new Key(
    "areYouSureYouEvenRegisteredYourEmailOnLichess")
  ...
  val `youCanAlsoScrollOverTheBoardToMoveInTheGame` = new Key(
    "youCanAlsoScrollOverTheBoardToMoveInTheGame")
  val `pressShiftPlusClickOrRightClickToDrawCirclesAndArrowsOnTheBoard` = new Key(
  ...
  def keys =
    List(
      ...
      `areYouSureYouEvenRegisteredYourEmailOnLichess`,
      `itWasNotRequiredForYourRegistration`,
      `weHaveSentYouAnEmailTo`,
      `byRegisteringYouAgreeToBeBoundByOur`,
      `networkLagBetweenYouAndLichess`,
      `timeToProcessAMoveOnLichessServer`,
      `downloadAnnotated`,
      `downloadRaw`,
      `downloadImported`,
      `printFriendlyPDF`,
      ...
      `playChessEverywhere`,
      `asFreeAsLichess`,
      `builtForTheLoveOfChessNotMoney`,
