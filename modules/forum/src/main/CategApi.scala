...
private[forum] final class CategApi(env: Env) {
  ...
  def makeTeam(slug: String, name: String): Funit =
    CategRepo.nextPosition flatMap { position =>
      val categ = Categ(_id = teamSlug(slug),
                        name = name,
      ...
      val post = Post.make(
        ...
        lang = "en".some,
        categId = categ.id)
      env.categColl.insert(categ).void >> env.postColl.insert(post).void >> env.topicColl
        .insert(topic withPost post)
        .void >> env.categColl.update($id(categ.id), categ withTopic post).void
    }
