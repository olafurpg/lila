...
private[forum] final class TopicApi(env: Env,
  ...
  def makeTopic(categ: Categ, data: DataForm.TopicData)(implicit ctx: UserContext): Fu[Topic] =
    TopicRepo.nextSlug(categ, data.name) zip detectLanguage(data.post.text) flatMap {
      case (slug, lang) =>
        ...
        val post = Post.make(topicId = topic.id,
                             ...
                             number = 1,
                             categId = categ.id)
        env.postColl.insert(post) >> env.topicColl.insert(topic withPost post) >> env.categColl
          .update($id(categ.id), categ withTopic post) >>- (indexer ! InsertPost(post)) >> env.recent.invalidate >>- ctx.userId.?? {
          userId =>
          ...
          (ctx.userId ifFalse post.troll) ?? { userId =>
            timeline ! Propagate(ForumPost(userId, topic.id.some, topic.name, post.id)).|>(
              prop => post.isStaff.fold(prop toStaffFriendsOf userId, prop toFollowersOf userId))
          }
  ...
  def paginator(categ: Categ, page: Int, troll: Boolean): Fu[Paginator[TopicView]] =
    Paginator(adapter = new Adapter[Topic](
  ...
  def toggleClose(categ: Categ, topic: Topic, mod: User): Funit =
    TopicRepo.close(topic.id, topic.open) >> {
      MasterGranter(_.ModerateForum)(mod) ?? modLog.toggleCloseTopic(
        mod.id, categ.name, topic.name, topic.open)
    }
  ...
  def toggleHide(categ: Categ, topic: Topic, mod: User): Funit =
    TopicRepo.hide(topic.id, topic.visibleOnHome) >> {
      MasterGranter(_.ModerateForum)(mod) ?? {
        PostRepo.hideByTopic(topic.id, topic.visibleOnHome) zip modLog.toggleHideTopic(
          mod.id, categ.name, topic.name, topic.visibleOnHome)
