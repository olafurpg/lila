...
final class PostApi(env: Env,
    ...
    lastNumberOf(topic) zip detectLanguage(data.text) zip userIds(topic) flatMap {
      case ((number, lang), topicUserIds) =>
        ...
        PostRepo findDuplicate post flatMap {
          ...
          case _ =>
            env.postColl.insert(post) >> env.topicColl.update($id(topic.id), topic withPost post) >> {
              shouldHideOnPost(topic) ?? TopicRepo.hide(topic.id, true)
            } >> env.categColl.update($id(categ.id), categ withTopic post) >>-
            (indexer ! InsertPost(post)) >> (env.recent.invalidate inject post) >>- ctx.userId.?? {
              userId =>
  ...
  private def shouldHideOnPost(topic: Topic) =
    topic.visibleOnHome && {
      (quickHideCategs(topic.categId) && topic.nbPosts == 1) || {
        topic.nbPosts == maxPerPage || topic.createdAt.isBefore(DateTime.now minusDays 5)
      }
    }
  ...
  def paginator(topic: Topic, page: Int, troll: Boolean): Fu[Paginator[Post]] =
    Paginator(new Adapter(collection = env.postColl,
  ...
  def delete(categSlug: String, postId: String, mod: User): Funit =
    (for {
      ...
      _ ← first.fold(env.topicApi.delete(view.categ, view.topic),
                     env.postColl.remove(view.post) >> (env.topicApi denormalize view.topic) >>
