...
final class Env(config: Config, lifecycle: play.api.inject.ApplicationLifecycle) {
  ...
  lazy val db = {
    ...
    parsedUri.db.fold[DefaultDB](sys error s"cannot resolve database from URI: $parsedUri") {
      dbUri =>
...
object Env {
  ...
  lazy val current =
    "db" boot new Env(
      config = lila.common.PlayApp loadConfig "mongodb", lifecycle = lila.common.PlayApp.lifecycle)
}
