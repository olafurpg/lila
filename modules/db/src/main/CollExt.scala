...
trait CollExt { self: dsl with QueryBuilderExt =>
  ...
  final implicit class ExtendColl(coll: Coll) {
    ...
    def updateFieldUnchecked[V : BSONValueWriter](
        selector: BSONDocument, field: String, value: V) =
