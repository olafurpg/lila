...
final class QaApi(questionColl: Coll,
  ...
  object question {
    ...
    def edit(data: QuestionData, id: QuestionId): Fu[Option[Question]] = findById(id) flatMap {
      _ ?? { q =>
        ...
        questionColl.update($doc("_id" -> q2.id), q2) >> tag.clearCache >> relation.clearCache inject q2.some
      }
    }
    ...
    def findByIds(ids: List[QuestionId]): Fu[List[Question]] =
      questionColl
        .find($doc("_id" -> $doc("$in" -> ids.distinct)))
        .cursor[Question]()
        .gather[List]()
    ...
    private def paginator(
        selector: Bdoc, sort: Bdoc, page: Int, perPage: Int): Fu[Paginator[Question]] =
      Paginator(adapter = new Adapter[Question](
    ...
    private def popularCache =
      mongoCache[Int, List[Question]](prefix = "qa:popular",
                                      f = nb =>
                                          questionColl
                                            .find($empty)
    ...
    def byTags(tags: List[String], max: Int): Fu[List[Question]] =
      questionColl
        .find($doc("tags" -> $doc("$in" -> tags.map(_.toLowerCase))))
        .cursor[Question]()
        .gather[List](max)

    def addComment(c: Comment)(q: Question) =
      questionColl.update($doc("_id" -> q.id), $doc("$push" -> $doc("comments" -> c)))
    ...
    def incViews(q: Question) =
      questionColl.update($doc("_id" -> q.id), $doc("$inc" -> $doc("views" -> BSONInteger(1))))
    ...
    def remove(id: QuestionId) =
      questionColl.remove($doc("_id" -> id)) >> (answer removeByQuestion id) >> tag.clearCache >> relation.clearCache
  ...
  object answer {
    ...
    def create(data: AnswerData, q: Question, user: User): Fu[Answer] =
      lila.db.Util findNextId answerColl flatMap { id =>
        ...
        (answerColl insert a) >> (question recountAnswers q.id) >>- notifier.createAnswer(
          q, a, user) inject a
      }
    ...
    def popular(questionId: QuestionId): Fu[List[Answer]] =
      answerColl
        .find($doc("questionId" -> questionId))
        .sort($doc("vote.score" -> -1))
        .cursor[Answer]()
        .gather[List]()
    ...
    def addComment(c: Comment)(a: Answer) =
      answerColl.update($doc("_id" -> a.id), $doc("$push" -> $doc("comments" -> c)))
    ...
    def remove(a: Answer): Fu[Unit] =
      answerColl.remove($doc("_id" -> a.id)) >> (question recountAnswers a.questionId).void
    ...
    def removeComment(id: QuestionId, c: CommentId) =
      answerColl.update($doc("questionId" -> id),
                        $doc("$pull" -> $doc("comments" -> $doc("id" -> c))),
                        multi = true)
  ...
  object comment {
    ...
    def remove(questionId: QuestionId, commentId: CommentId) =
      question.removeComment(questionId, commentId) >> answer.removeComment(questionId, commentId)
  ...
  object tag {
    ...
    def all: Fu[List[Tag]] = cache(true) {
      ...
      col
        .aggregate(Project($doc("tags" -> BSONBoolean(true))),
                   List(Unwind("tags"), Group(BSONBoolean(true))("tags" -> AddToSet("tags"))))
