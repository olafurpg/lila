...
case class AnaDrop(role: chess.Role, pos: chess.Pos, variant: Variant, fen: String, path: String) {
...
object AnaDrop {
  ...
  def parse(o: JsObject) =
    for {
      ...
      fen ← d str "fen"
      path ← d str "path"
    } yield AnaDrop(role = role, pos = pos, variant = variant, fen = fen, path = path)
}
