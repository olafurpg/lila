...
private[analyse] case class CpAdvice(nag: Nag, info: Info, prev: Info) extends Advice
...
private[analyse] object CpAdvice {
  ...
  private val cpNags = List(300 -> Nag.Blunder, 100 -> Nag.Mistake, 50 -> Nag.Inaccuracy)
...
private[analyse] case class MateAdvice(sequence: MateSequence, nag: Nag, info: Info, prev: Info)
    extends Advice
